...
export default Vue.extend({
  ...
  methods: {
    ...
    async loadTraCuuVatTuThueBao() {
      await this.loadDSDichVuVienThong();
      let dsTS = await this.getDSThamSoMacDinhTheoMaTS({maTS: "DHSX_SOHOA_QLVT"});
      this.bienDongVTIsVisible = dsTS.length > 0
      this.Visible.colMDC = dsTS.length > 0
    },
    ...
    async onChangeDichVu(row) {
      ...
      if(row != null && this.p_dich_vu_id !=  row.dichVuId){
    ...
    async openPopupXemAnh(item){      
      ...
      let data = await this.getHinhAnh(item.PHIEUVT_ID)
      if(data == null){
