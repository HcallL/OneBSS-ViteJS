...
export default {
  ...
  mounted: async function () {
    console.log("HandoverTicket mounted");
    ...
    if (this.$route.query.trangthaiphieu) {
      this.listbox.trangthaiphieu.value.id =
        this.$route.query.trangthaiphieu == ""
          ? 1
          : this.$route.query.trangthaiphieu;
    }
    //Loại hợp đồng
  ...
  data() {
    return {
      ...
      ngay_ht: new Date(),
      header: {
        title: "GIAO PHIÊU ĐIỀU HÀNH THI CÔNG",
        list: [
  ...
  methods: {
      ...
      API.lay_ds_thamso_md_mats(this.axios, input).then((response) => {
        if (
          ...
          if (this.listbox.thamso.list.length > 0) {
            var temp = this.listbox.thamso.list.filter(
              (x) => x.ma_ts === "NHANTIN_NHANVIEN_PHOIHOP"
            );
            if (temp.length > 0) {
              //console.log(temp);
            ...
            var temp1 = this.listbox.thamso.list.filter(
              (x) => x.ma_ts === "THEM_TT_HDKHDN"
            );
            if (temp1.length > 0) {
              //console.log(temp1);
            ...
            var temp2 = this.listbox.thamso.list.filter(
              (x) => x.ma_ts === "KT_PHIEUTRA_GDV"
            );
            if (temp2.length > 0) {
              //console.log(temp2);
            ...
            var temp3 = this.listbox.thamso.list.filter(
              (x) => x.ma_ts === "SEND_SMS_NHANTIN_NV"
            );
            if (temp3.length > 0) {
              //console.log(temp2);
            ...
            var temp4 = this.listbox.thamso.list.filter(
              (x) => x.ma_ts === "NHANTIN_THEO_HG"
            );
            if (temp4.length > 0) {
              //console.log(temp2);
      ...
      try {
        ...
        API.CSS_LOAI_HD(this.axios, input).then((response) => {
          if (
            ...
            this.listbox.loaihd.list = response.data.data
              .filter((x) => x.LOAIHD_ID > 0)
              .map((x) => ({ id: x.LOAIHD_ID, text: x.TEN_LOAIHD }));
          } else {
      ...
      try {
        ...
        API.CSS_DICHVU_VT(this.axios, input).then((response) => {
          if (
            ...
            this.listbox.dichvu.list = response.data.data
              .filter((x) => x.DICHVUVT_ID > 0)
              .map((x) => ({ id: x.DICHVUVT_ID, text: x.TEN_DVVT }));
          } else {
    ...
    LoadQuyTrinh() {
      ...
      try {
            ...
            this.listbox.quytrinh.list = response.data.data
              .filter(
                (x) =>
                  x.quytrinh_id > 0 &&
                  x.loaihd_id ==
                    (this.listbox.loaihd.value.LOAIHD_ID == null
                      ? 0
                      : this.listbox.loaihd.value.LOAIHD_ID) &&
                  x.dichvuvt_id ==
                    (this.listbox.dichvu.value.DICHVUVT_ID == null
                      ? 0
                      : this.listbox.dichvu.value.DICHVUVT_ID)
              )
              .map((x) => ({ id: x.quytrinh_id, text: x.quytrinh }));
            console.log(this.listbox.quytrinh.list);
            if (this.listbox.quytrinh.list.length > 0) {
              this.listbox.quytrinh.value.quytrinh_id =
                this.listbox.quytrinh.list[0].id;
            }
          } else {
        ...
        this.ShowAlert("Lỗi lấy thông tin quy trình!", 2);
        this.loading(false);
      }
    },
    ...
    LoadHuongGiao: async function () {
      ...
      try {
        ...
        input.quytrinh_id = this.isTimTheoQuyTrinh
          ? this.listbox.quytrinh.value.quytrinh_id
          : 0;
        input.loaihd_id = this.isTimTheoLoaiHD
          ? this.listbox.loaihd.value.LOAIHD_ID
          : 0;
        input.dichvuvt_id = this.isTimTheoDichVu
          ? this.listbox.dichvu.value.DICHVUVT_ID
          : 0;
        ...
        if (this.isTimTheoQuyTrinh) {
          input.nhom_qt_id =
            this.listbox.quytrinh.value.nhom_qt_id == null
              ? 1
              : this.listbox.quytrinh.value.nhom_qt_id;
          input.quytrinh_id =
            this.listbox.quytrinh.value.quytrinh_id == null
              ? 0
              : this.listbox.quytrinh.value.quytrinh_id;
        }
        //console.log(input);
        await API.sp_lay_ds_huonggiao(this.axios, input).then((response) => {
          if (
            ...
            this.listbox.huonggiao.list = response.data.data
              .filter((x) => x.huonggiao_id > 0)
              .map((x) => ({ id: x.huonggiao_id, text: x.huonggiao }));
            //console.log(this.listbox.huonggiao.list);
            if (this.listbox.huonggiao.list.length > 0) {
              if (
                this.$route.query.huonggiao &&
                this.$route.query.huonggiao != ""
              ) {
                this.listbox.huonggiao.value.huonggiao_id =
                  this.$route.query.huonggiao;
              } else {
                if (this.DICHVU == 1) {
                  //dịch vụ cố định
                  this.listbox.huonggiao.value.huonggiao_id =
                    this.HUONG_LAPMOI_CD;
                } else if (this.DICHVU == 4) {
                  this.listbox.huonggiao.value.huonggiao_id =
                    this.HUONG_LAPMOI_FTTH;
                } else if (this.DICHVU == 10) {
                  this.listbox.huonggiao.value.huonggiao_id =
                    this.HUONG_LAPMOI_MYTV;
                }
              }
    ...
    HienThi_DanhSach() {
      ...
      if (this.listbox.trangthaiphieu.value.id == "1") {
        ...
        try {
          ...
          input.dichvuvt_id = this.isTimTheoDichVu
            ? this.listbox.dichvu.value.DICHVUVT_ID
            : 0; // ((this.listbox.dichvu.value.DICHVUVT_ID ==null || this.isTimTheoLoaiHD == false)? 0: this.listbox.dichvu.value.DICHVUVT_ID);
          input.loaihd_id = this.isTimTheoLoaiHD
            ? this.listbox.loaihd.value.LOAIHD_ID
            : 0; //(this.listbox.loaihd.value.LOAIHD_ID == null? 0: this.listbox.loaihd.value.LOAIHD_ID);
          input.huonggiao_id =
            this.listbox.huonggiao.value.huonggiao_id == null
              ? 0
              : this.listbox.huonggiao.value.huonggiao_id;
          ...
          API.sp_lay_ds_hdtb_chuagiao(this.axios, input).then((response) => {
            ...
            if (
              ...
              if (this.txtMaGD != "") {
                this.listbox.ds_phieuyeucau.list =
                  this.listbox.ds_phieuyeucau.list.filter(
                    (x) => x.ma_gd == this.txtMaGD
                  );
          ...
          this.ShowAlert("Có lỗi lấy danh sách phiếu chưa giao!", 2);
          this.loading(false);
        }
      } else if (this.listbox.trangthaiphieu.value.id == "2") {
        //đã giao
        try {
          ...
          this.ShowAlert("Có lỗi lấy danh sách phiếu đã giao!", 2);
          this.loading(false);
        }
      }
    },
        ...
        API.fn_lay_loaidv_dich_theo_hg(this.axios, input).then((response) => {
          if (
            ...
            if (this.loaidv_nhan_id != "-1") {
              ...
              console.log(input1);
              API.sp_ht_tatca_vetinh_combobox_v2(this.axios, input1).then(
                (response) => {
                  if (
                    response.data.error_code &&
                    response.data.error_code === this.success_code
                  ) {
                    this.listbox.donvinhan.list = response.data.data.map(
                      (x) => ({ id: x.donvi_id, text: x.ten_dv })
                    );
    ...
    btnHuyGiao() {
      ...
      var temp = [];
      temp = listphieuchuyen.map((x) => ({
        CHON: 1,
        PHIEU_ID: x.PHIEU_ID,
        HDTB_ID: x.HDTB_ID,
        MA_TB: x.MA_TB,
        TTPH_ID: x.TTPH_ID,
        LOAIPT_ID: x.LOAIPT_ID,
        STATUS: x.STATUS,
        PHIEU_CHA_ID: x.PHIEU_CHA_ID,
      }));
      try {
        var input = {};
    ...
    async btnCapNhatDV() {
      if (
        this.listbox.donvinhan.value.donvi_id == null ||
      ...
      for (var item of listphieuchuyen) {
        if (this.loaidv_nhan_id != "-1") {
          try {
            ...
            input1.loaidv_id = this.loaidv_nhan_id;
            if (this.listbox.dichvu.value.DICHVUVT_ID != this.DV_BTSL) {
              //nếu là dịch vụ TSL
              input1.kieudv_id = 2;
            } else {
              input1.kieudv_id = 1;
    ...
    btnXuatFile: function () {
      ...
      if (
        this.listbox.ds_phieuyeucau.list != null &&
        this.listbox.ds_phieuyeucau.list.length > 0
      ) {
    ...
    gridChanged(obj) {
      ...
      if (obj.dichvuvt_id == this.DV_BTSL) {
        if (obj.danhdau == "3") {
          //Đơn vị đầu trùng đơn vị cuối
          this.kt_daucuoi = 1;
        } // Đơn vị đầu khác đơn vị cuối
        else {
          this.kt_daucuoi = 0;
        }
      } else {
    ...
    HienThi_DanhSach_v1() {
      if (this.listbox.trangthaiphieu.value.id == "1") {
        ...
        try {
          ...
          API.sp_lay_ds_hdtb_chuagiao(this.axios, input).then((response) => {
            ...
            if (
              ...
              if (this.txtMaGD != "") {
                this.listbox.ds_phieuyeucau.list =
                  this.listbox.ds_phieuyeucau.list.filter(
                    (x) => x.ma_gd == this.txtMaGD || x.ma_tb == this.txtMaGD
                  );
                if (this.listbox.ds_phieuyeucau.list.length <= 0) {
                  this.ShowAlert("Không tìm thấy phiếu", 2);
                }
          ...
          this.ShowAlert("Lỗi lấy danh sách phiếu chưa giao", 2);
          this.loading(false);
        }
      } else if (this.listbox.trangthaiphieu.value.id == "2") {
        //đã giao
        try {
          ...
          input.huonggiao_id =
            this.listbox.huonggiao.value.huonggiao_id == null
              ? 0
              : this.listbox.huonggiao.value.huonggiao_id; //7;
          input.donvi_id = this.$root.token.getDonViID();
          input.loaipt_id = 0;
          ...
          this.ShowAlert("Lỗi lấy danh sách phiếu đã giao", 2);
          this.loading(false);
        }
      }
    },
