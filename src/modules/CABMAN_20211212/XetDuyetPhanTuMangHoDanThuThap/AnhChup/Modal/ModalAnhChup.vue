    ...
    loadHtmlItems(sources){
      if (sources.length > 0){
          sources.forEach(item =>{
              let component ={
                template:
                  '                <div class="box-file-image-upload bss1833-box-file-image" style="max-width: 600px;margin: auto;">\n' +
                  ...
                  '                                            <span class="file-text" v-show="!item.hasImage">Click double để chọn ảnh</span>\n' +
                  '                  </div>\n' +
                  '                </div>',
                data(){
                    return {
