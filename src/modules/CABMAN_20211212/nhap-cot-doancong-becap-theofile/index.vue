...
export default {
  ...
  methods: {
    ...
    convertToCotItem: async function (dataItem) {
      ...
      let duAnID = null;
      let taiSanID = null;
      if (!this.isNullOrEmpty(dataItem.R) && !this.isNullOrEmpty(dataItem.Q)) {
    ...
    convertToCongItem: async function (dataItem) {
      ...
      let duAnID = null;
      let taiSanID = null;
      if (!this.isNullOrEmpty(dataItem.N) && !this.isNullOrEmpty(dataItem.M)) {
        let v = await this.getTaiSanDuAnTheoMa(dataItem.N, dataItem.M);
        if (v != null) {
          duAnID = v.DUAN_ID;
          taiSanID = v.TAISAN_ID;
        } else {
    ...
    convertToBeItem: async function (dataItem) {
      ...
      let duAnID = null;
      let taiSanID = null;
      if (!this.isNullOrEmpty(dataItem.U) && !this.isNullOrEmpty(dataItem.T)) {
        let v = await this.getTaiSanDuAnTheoMa(dataItem.U, dataItem.T);
        if (v != null) {
          duAnID = v.DUAN_ID;
          taiSanID = v.TAISAN_ID;
        } else {
    ...
    themCot: async function (dataItem) {
      ...
      let rs = await this.$root.context.post(
        "/web-cabman/cot-cong-be/them-cot",
        data
      );
      return rs.data;
    },
    ...
    themCong: async function (dataItem) {
      ...
      let rs = await this.$root.context.post(
        "/web-cabman/cot-cong-be/them-doancong",
        data
      );
      return rs.data;
    },
              ...
              for (let i = 0; i < workBook.SheetNames.length; i++) {
                ...
                switch (sheetName.toUpperCase()) {
                  ...
                  case "BE":
                    ...
                    break;
                  // case "LOẠI BỂ":
                  //   this.dsLoaiBe = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsLoaiBe.shift(); // remove first item
                  //   break;
                  // case "VỊ TRÍ ĐẶT BỂ":
                  //   this.dsViTri = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsViTri.shift(); // remove first item
                  //   break;
                  // case "HƯỚNG TẤM ĐAN":
                  //   this.dsHuongTamDan = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsHuongTamDan.shift(); // remove first item
                  //   break;
                  // case "LOẠI ĐƯỜNG":
                  //   this.dsLoaiDuong = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsLoaiDuong.shift(); // remove first item
                  //   break;
                  // case "LOẠI CỐNG":
                  //   this.dsLoaiCong = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsLoaiCong.shift(); // remove first item
                  //   break;
                  // case "LOẠI CỘT":
                  //   this.dsLoaiCot = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsLoaiCot.shift(); // remove first item
                  //   break;
                  // case "CHỦ QUẢN":
                  //   this.dsChuQuan = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsChuQuan.shift(); // remove first item
                  //   break;
                  // case "KIỂU CỘT":
                  //   this.dsKieuCot = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsKieuCot.shift(); // remove first item
                  //   break;
                  // case "TRẠNG THÁI SỬ DỤNG":
                  //   this.dsTTSD = this.getTextValues(
                  //     xlsx.utils.sheet_to_json(workBook.Sheets[sheetName], {
                  //       header: "A",
                  //     })
                  //   );
                  //   this.dsTTSD.shift(); // remove first item
                  //   break;
                  default:
                    break;
                }
