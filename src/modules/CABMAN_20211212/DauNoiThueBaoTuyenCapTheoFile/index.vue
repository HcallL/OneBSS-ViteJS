    ...
    async getdulieu() {
      ...
      try {
        ...
        if (dsData[0].length > 0){
          ...
          .then(res =>{
            rs = res.data
          }).catch(e =>this.$root.toastError("Có lỗi xảy ra trong quá trình lấy thông tin thuê bao!"))
          if (rs.length > 0){
    ...
    btnDauNoi: async function (){
      ...
      try {
        ...
        for (let [index,item] of dsDauNoi.entries()) {
          ...
          await this.$root.context.get("/web-cabman/ketcuoi/lay-id-theo-kyhieu",{kyhieu_kc:ketcuoi})
          .then(res =>{
