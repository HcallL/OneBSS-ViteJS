  ...
  <div style="height: 800px;">
      ...
      <GmapMap
            ...
            style="width: 100%; height: calc(100% - 26px);">
              ...
              <GmapMarker
                  v-for="(node, index) in getMarkers"
                  :key="index+'m1'"
              ...
              <GmapMarker
                  v-for="(node, index) in getMarkersInArea"
                  :key="index+'m2'"
                  :position="node.position"
                  :icon="getMarkerIcon()"
              >
              ...
              <gmap-circle
                  ...
                  :radius="pin.r"
                  :visible="true"
                  :options="{fillOpacity:0.5,strokeWeight: 0.5,  fillColor: '#5cc7ea'}">
              </gmap-circle>
              <GmapMarker
                :key="index+'m3'"
...
export default {
      ...
      for (let index = 0; index < this.markers.length; index++) {
        const marker = this.markers[index]
          this.nodes.push({
            ...
            icon: marker.LOAI == 1 ? this.getMarkerIcon2() : this.getMarkerIconKC()
          })
      }
      //console.log('getMarkers nodes', this.nodes)
      return this.nodes
    },
    getMarkersInArea () {
      let nodes = []
      for (let index = 0; index < this.dsTBLanCan.length; index++) {
        const marker = this.dsTBLanCan[index];
          nodes.push({
  ...
  methods: {
    searchLocation: function (searchAddressInput) {
      // var lat = parseFloat(searchAddressInput.split(',')[0])
      // var lng = parseFloat(searchAddressInput.split(',')[1])
      // console.log('lat ',lat)
      // console.log('lng ',lng)
    ...
    rightclickMap (event) {
      ...
      console.log('rightclickMap', event.latLng)
    ...
    getMarkerIconKC () {
      return {
        url: require('../../../../../static/icons/icon100x100/KET_CUOI_AO.jpg'),
