  ...
  methods: {
    ...
    async onShownModal() {
      ...
      try {
        ...
        if (this.midspanObject.MIDSPAN_ID){
          ...
          if (this.midspan.DONVI_ID && this.midspan.DONVI_TD_ID){
            await this.getTenTramThietBi(this.midspan.DONVI_ID,"NGUON")
            await this.getTenTramThietBi(this.midspan.DONVI_TD_ID,"DICH")
          }
          if (this.midspan.KETCUOI_N_ID && this.midspan.KETCUOI_D_ID){
            await this.getTenKetCuoi(this.midspan.KETCUOI_N_ID, "NGUON")
            await this.getTenKetCuoi(this.midspan.KETCUOI_D_ID, "DICH")
          }
        }
      }catch (e){}
      finally {
    ...
    async getTenTramThietBi(donviId, nguondich){
      await this.$root.context.get(
        ...
        if (res.data && nguondich === "DICH") this.midspan.TEN_DV_TD = res.data.TEN_DV_TD
      }).catch(e=> {
        this.$toast.error("Có lỗi xảy ra khi tải kiểu cáp")
      })
    },
