...
export default Vue.extend({
      ...
      contextMenuOpen: function (args) {
        if (this.selectedItems.nodes.length > 0) {
          ...
          if (diagramInstance.selectedItems.nodes[0].Group === "KETCUOI") {
            ...
            diagramInstance.selectedItems.nodes[0].Group === "TRAM_TB"
          ) {
          ...
          if (diagramInstance.layout.type !== null) {
            let them_diemuon = document.getElementById(
              "MENU_ITEM_THEM_DIEMUON_CAP"
            );
            ...
            args.hiddenItems.push("MENU_ITEM_XOA_DIEMUON_CAP");
            let xoa_diemuon = document.getElementById(
              "MENU_ITEM_XOA_DIEMUON_CAP"
            );
            ...
            if (
              ...
              !(
                ...
                !args.event.target.classList.contains(
          ...
          if (diagramInstance.layout.type !== null) {
            let luu_bocuc = document.getElementById(
              "MENU_ITEM_LUU_BOCUC_GIANDO"
            );
  ...
  methods: {
    ...
    contextMenuClick: function (args) {
      ...
      if (args.element.id === "ICON_HOST") {
        let idx = diagramInstance.nodes.findIndex(
          (x) =>
            x.id.toString() ===
            diagramInstance.selectedItems.nodes[0].id.toString()
        );
        if (idx > -1) {
        ...
        this.$refs.ModalTracnghiemExt.show(tracnghiem).then((result) => {
          ...
          switch (result.option) {
            case 0:
              ...
              this.$refs.ModalChonDSKcExt.show().then((result) => {
                let data = {};
                data.KETCUOI_ID = result.doituong_id;
                data.CAP_ID = parseInt(
                  diagramInstance.selectedItems.connectors[0].id
                );
    ...
    async quanLyKetCuoi(ketcuoi_id) {
      ...
      if (ketcuoi) {
        this.$refs.QLKetCuoiGianDo.show(false, ketcuoi).then(async (result) => {
          if (result.ok) {
    ...
    mniThemDiemUonGD_Click(position) {
      ...
      if (idx > 1) {
        ...
        diemuon_list.unshift({x:cap.sourcePoint.x, y:cap.sourcePoint.y})
        diemuon_list.push({x:cap.targetPoint.x, y:cap.targetPoint.y})
    ...
    async mniDoiBieuTuong_Click(TRAMTB_ID, ICON) {
      if (TRAMTB_ID && ICON) {
        await this.$root.context
          ...
          .catch((e) =>
            this.$root.toastError("Có lỗi xảy ra khi đổi biểu tượng")
          );
      }
    },
    // ĐỔI ĐÁNH DẤU
    ...
    mniTaoKetCuoi_Click() {
      ...
      point.DONVI_ID = -1;
      point.CHON_TOQL = true;
      this.$refs.QLKetCuoiGianDo.show(true, point).then(async (result) => {
        if (result.ok) {
          console.log(result.data);
          console.log(point);
          point.id = result.data.CAP_ID;
          // tạo node kết cuối
          await this.$root.context
    ...
    mniXoaTBDU_GianDo_Click() {
      this.$bvModal.msgBoxConfirm("Bạn có muốn xóa toàn bộ điểm uốn trên giản đồ không?",
        ...
        if (value){
          diagramInstance.connectors.map((item) => ({ ...item, segments: [] }));
          diagramInstance.resetSegments();
          diagramInstance.dataBind();
        }
        else{
    ...
    CapNhatLink(cap) {
      ...
      if (idx > -1) {
        ...
        diagramInstance.connectors[idx].SelectedBackColor =
          cap.SelectedBackColor;
        diagramInstance.connectors[idx].SelectedDashStyle =
          cap.SelectedDashStyle;
    ...
    diagramClick(args) {
      ...
      if (this.previousNode) {
        let idx = diagramInstance.nodes.findIndex(
          (x) => x.id.toString() === this.previousNode.id.toString()
        );
        if (idx > -1) {
          diagramInstance.nodes[idx].shape.source = diagramInstance.nodes[
            idx
          ].shape.source.replace("_sel", "");
      ...
      if (this.previousConnector) {
        ...
        if (idx > -1) {
          diagramInstance.connectors[idx].style.strokeWidth =
            this.previousConnector.Width;
          diagramInstance.connectors[idx].style.strokeColor =
            this.previousConnector.BackColor;
      ...
      if (clickedItem) {
        ...
        if (clickedItem.propName === "connectors") {
          ...
          if (idx > -1) {
            ...
            // layout custom
            if (diagramInstance.layout.type === null)
              diagramInstance.connectors[idx].constraints =
                (ConnectorConstraints.Default |
                  ConnectorConstraints.DragSegmentThumb) &
                ~ConnectorConstraints.Drag; /*&  ~ConnectorConstraints.DragTargetEnd & ~ConnectorConstraints.DragSourceEnd*/
            else
              diagramInstance.connectors[idx].constraints =
                ConnectorConstraints.Tooltip |
                ~ConnectorConstraints.Drag &
        ...
        if (clickedItem.propName === "nodes") {
          ...
          if (idx > -1) {
            ...
            // layout custom
            if (diagramInstance.layout.type === null)
              diagramInstance.nodes[idx].constraints =
                (NodeConstraints.Default |
                  NodeConstraints.Tooltip |
                  NodeConstraints.Drag) &
                ~NodeConstraints.Resize;
          ...
          //Tạo cáp tới bằng (double click: fromObj, click left:toObj)
          if (args.button === "Left" && this.fromObjDoubleClick !== null) {
            let toObj = clickedItem;
            let fromObj = diagramInstance.nodes.filter(
              (item) => item.id === this.fromObjDoubleClick
            )[0];
            this.$emit("diaGianDo_ObjectAdded", "Connector", fromObj, toObj);
            this.fromObjDoubleClick = null;
          }
        }
        diagramInstance.dataBind();
    ...
    selectConnector(id){
      ...
      if (connector !== null){
        let bound = new Rect(connector.targetPoint.x ,connector.targetPoint.y, 600, 600);
    ...
    bringIntoView(translate){
      this.currentOffset = {x:translate.x, y:translate.y}
