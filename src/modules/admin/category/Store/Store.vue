          ...
          <div class="row">
              <Split>
                  <SplitArea :size="20" :minSize="100">
                      <store-list  ref="storeList"  :storesData="storesData" @onSelect="onSelectStore"></store-list>
                  </SplitArea>
                  <SplitArea :size="80" :minSize="100">
                     <div class="row">
                      <div class="grid-stack-box">
                        <ejs-splitter id='splitter_2' class='splitterContent' ref="splitterObj_2" :separatorSize=1 width='100%'>
                          <e-panes>
                            <e-pane size="40%" min="20%" content='#content111'></e-pane>
                            <e-pane size="60%" min="20%" content='#content112'></e-pane>
                          </e-panes>
                        </ejs-splitter>
                      </div>
                      <div id='content111'>
...
methods: {
  ...
  try {
    const parent_id = this.getSelectedStore.kho_cha_id
    if (this.isValidForm()) {
      ...
      if (updatedStoreDataResult.isSuccess) {
        ...
        if (this.isEnableAddNew){
          this.expandNode(roots[0], this.storeInfo.selectedParentStoreId)
        }
        else {          
          const MA_KHO = this.storeInfo.prefixStoreCode !== '' &&
              this.storeInfo.prefixStoreCode !== undefined && this.storeInfo.prefixStoreCode !== null
              ? `${this.storeInfo.prefixStoreCode}.${this.storeInfo.storeCode}` : this.storeInfo.storeCode;
          const selectedStore = stores.find(i => i.ma_kho === MA_KHO)            
          this.setSelectedStore(selectedStore)
          this.$refs.storeList.setSelectedValue(selectedStore.kho_id,stores)
          this.expandNode(roots[0], selectedStore.kho_id)
        }
      } else {
  ...
  deleteStore: async function () {
    ...
    try {
      ...
      if (resCheckDataResult.isSuccess) {
        ...
        this.resetStoreInfo()
        if (parent_id)
          this.expandNode(roots[0], parent_id)
      } else {
  ...
  isValidForm: function () {
    ...
    const store = this.getStores.find(i => i.ma_kho === this.storeInfo.prefixStoreCode+'.'+this.storeInfo.storeCode)
    if (isAddNew && store) {
      this.$toast.error('Mã kho '+this.storeInfo.prefixStoreCode+'.'+this.storeInfo.storeCode+' đã tồn tại !')
      this.$refs.storeInfo.isStoreCodeError = true
      return false
    } else { this.$refs.storeInfo.isStoreCodeError = false }
