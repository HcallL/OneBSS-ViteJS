...
export default {
  ...
  methods: {
      ...
      if (!this.isNullOrEmpty(item.KYHIEU_KETCUOI_CHA)) {
        ...
        if (!kcMap) {
          ...
          if (ketcuoi && ketcuoi.KETCUOI_ID) {
            ...
            item.MESSAGE.ketcuoi_cha_id = 'Kết cuối cha không tồn tại. Kiểm tra lại ký hiệu kết cuối cha trong file nhập vào'
          }
        } else {
    ...
    validateCap: async function (item) {
      ...
      console.log('🚀 ~ file: index.vue:1490 ~ this.isNullOrEmpty(item.KETCUOI_N):', this.isNullOrEmpty(item.KETCUOI_N))
      if (!this.isNullOrEmpty(item.KETCUOI_N)) {
        if (item.LOPMC_ID + '' == '1') {
          ...
          if (ketcuoi_n) {
            ...
            item.KETCUOI_N_ID = null
            item.MESSAGE.KETCUOI_N_ID = 'Không tồn tại kết cuối nguồn với ký hiệu như trong file hoặc có tồn tại nhưng không phù hợp với nhóm loại cáp (quang hoặc đồng)'
          }
        }
      } else {
        if (item.LOPMC_ID + '' != '1') {
          item.MESSAGE.KETCUOI_N_ID = 'Chưa có thông tin kết cuối nguồn. Hãy nhập ký hiệu kết cuối nguồn vào cột dữ liệu trong file rồi thử lại'
        }
      }
      ...
      if (!this.isNullOrEmpty(item.KETCUOI_D)) {
        ...
        if (item.LOPMC_ID + '' != '1') {
          item.MESSAGE.KETCUOI_D_ID = 'Chưa có thông tin kết cuối đích. Hãy nhập ký hiệu kết cuối đích vào cột dữ liệu trong file rồi thử lại'
        }
      }
      // //Gán lại tổ quản lý
      // if (this.isNullOrEmpty(item.DONVI_ID)){
      ...
      for (let i = 0; i < this.danhSachCap.length; i++) {
        ...
        try {
          ...
          if (e && e.response && e.response.data && e.response.data.message_detail) {
            ...
            if (e.response.data.message_detail.includes('Ký hiệu đã được sử dụng')) { this.danhSachCap[i].MESSAGE.kyhieu = e.response.data.message_detail }
            // this.danhSachCap[i].TRANGTHAI = e.response.data.message_detail
          } else {
            this.danhSachCap[i].TRANGTHAI = 'Lưu thất bại!. ' +
      ...
      for (let i = 0; i < this.danhSachKetCuoi.length; i++) {
        ...
        try {
          ...
          if (e && e.response && e.response.data && e.response.data.message_detail) {
            ...
            if (e.response.data.message_detail.includes('Phân loại kết cuối, Ký hiệu đã được sử dụng')) { this.danhSachKetCuoi[i].MESSAGE.kyHieu = e.response.data.message_detail }
          } else {
            this.danhSachKetCuoi[i].TRANGTHAI = 'Lưu thất bại!. ' +
              JSON.stringify(e.response.data.message || {}) +
              JSON.stringify(e.response.data.data || {})
