...
export default {
  ...
  methods: {
    linkTaiFileMau_Click: function () {
      window.location.href = "/static/files/MauFile_Update_KC_Cap.xlsx";
    },
      ...
      if (!(rows == null || rows.length < 2)) {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          let item = {
            ...
            O: this.getColumnValue(cols, "Tổ kỹ thuật quản lý (Nhập chính xác tên hoặc mã đơn vị)", row),
            ...
            R: this.getColumnValue(cols, "Toàn nhà (Nhập chính xác mã toàn nhà)", row),
    ...
    convertToKetCuoiItem: async function (dataItem, currentItem) {
      ...
      if (!this.isNullOrEmpty(dataItem.Q)) {
        let v = await this.getTaiSanDuAnTheoMa(dataItem.Q, this.isNullOrEmpty(dataItem.P) ? null : dataItem.P);
        if (v != null) {
          duAnID = v.DUAN_ID;
      ...
      let result = {
        ...
        KYHIEU: this.isNullOrEmpty(dataItem.B)
          ? currentItem.KYHIEU
          : dataItem.B,
        KYHIEU_MOI: this.isNullOrEmpty(dataItem.Y)
          ? currentItem.KYHIEU
          : dataItem.Y,
        ...
        TEN_KC: this.isNullOrEmpty(dataItem.C)
          ? currentItem.TEN_KC
          : dataItem.C,
        TEN_KC_MOI: this.isNullOrEmpty(dataItem.Z)
          ? currentItem.TEN_KC
          : dataItem.Z,
        ...
        MA_GOITHAU_IMS: this.isNullOrEmpty(dataItem.W) ? currentItem.MA_GOITHAU_IMS : dataItem.W,
        MA_HOPDONG_IMS: this.isNullOrEmpty(dataItem.X) ? currentItem.MA_HOPDONG_IMS : dataItem.X,
        MESSAGE: msg,
        TRANGTHAI: "Sẵn sàng",
        ITEM_TYPE: "KETCUOI",
        IS_EDIT: true,
      ...
      if (!(rows == null || rows.length < 2)) {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          let item = {
            ...
            D: this.getColumnValue(cols, "SODOIDAY", row),
    ...
    convertToCapItem: async function (dataItem, currentItem) {
      ...
      let taiSanText = null;
      if (!this.isNullOrEmpty(dataItem.M)) {
        let v = await this.getTaiSanDuAnTheoMa(dataItem.M, this.isNullOrEmpty(dataItem.L) ? null : dataItem.L);
        if (v != null) {
          duAnID = v.DUAN_ID;
          taiSanID = v.TAISAN_ID;
      ...
      let result = {
        ...
        TEN_CAP: this.isNullOrEmpty(dataItem.C)
          ? currentItem.TEN_CAP
          : dataItem.C,
        KYHIEU_MOI: this.isNullOrEmpty(dataItem.V)
          ? currentItem.KYHIEU
          : dataItem.V,
        TEN_CAP_MOI: this.isNullOrEmpty(dataItem.W)
          ? currentItem.TEN_CAP
          : dataItem.W,
        KICHCO_ID: kichCoID,
        ...
        MA_GOITHAU_IMS: this.isNullOrEmpty(dataItem.T) ? currentItem.MA_GOITHAU_IMS : dataItem.T,
        MA_HOPDONG_IMS: this.isNullOrEmpty(dataItem.U) ? currentItem.MA_HOPDONG_IMS : dataItem.U,
    ...
    getDonViTheoTen: async function (tenDonVi) {
      try {
        ...
        this.$root.toastError(
          "Lỗi truy cập đến đơn vị: " + tenDonVi
        );
        return null;
      }
    },
    getDuAnTheoSoHieu: async function (soHieuCT) {
      try {
        ...
        this.$root.toastError(
          "Lỗi truy cập đến dự án theo số hiệu: " + soHieuCT
        );
        return null;
      }
    },
    ...
    capNhatCap: async function (dataItem) {
      ...
      try {
        ...
        this.$root.toastError(
            "Lỗi cập nhật cáp ID " + data.cap_id + ": " + msg
          );
      }
      return null;
    },
    ...
    capNhatKetCuoi: async function (dataItem) {
      ...
      try {
        ...
        this.$root.toastError(
            "Lỗi cập nhật kết cuối ID " + data.ketcuoi_id + ": " + msg
          );
      }
      return null;
    },
