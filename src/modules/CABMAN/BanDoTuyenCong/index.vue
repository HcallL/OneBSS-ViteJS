...
export default {
  ...
  data: function () {
    return {
      ...
      dsBeToaDo: [],
      dsSearch: [],
      cboTimKiem: [{id: 1, name: 'Cống/Bể'}, {id: 5, name: 'Bể'}, {id: 6, name: 'Cống'}],
      dataSend: {},
      isSuccess: true,
      tuyenCongId: null,
      tuyenCongIdDoi: null,
  ...
  methods: {
    ...
    xoaCong: async function () {
      try {
        if (this.doanCongIdThaoTac != null) {
          ...
          } else { this.$toast.error('Xóa cống: ' + this.doanCongIdThaoTac + ' thất bại') }
        }
      } catch (error) {
    ...
    xoaBe: async function () {
      try {
        if (this.beIdThaoTac != null) {
          ...
          } else { this.$toast.error('Xóa bể thất bại') }
        }
      } catch (error) {
    ...
    daoChieuDoanCong: async function () {
      try {
        ...
        } else { this.$toast.error('Đảo chiều thất bại') }
      } catch (error) {
      } finally {
      }
    },
    ...
    doiBeCapA: async function () {
      try {
        ...
        } else { this.$toast.error('đổi bể cáp a thất bại') }
      } catch (error) {
      } finally {
      }
    },
    ...
    doiBeCapB: async function () {
      try {
        ...
        } else { this.$toast.error('đổi bể cáp b thất bại') }
      } catch (error) {
      } finally {
      }
    },
    ...
    chuyenTuyenBeCap: async function () {
      try {
        ...
        } else { this.$toast.error('chuyển tuyến bể cáp thất bại') }
      } catch (error) {
      } finally {
      }
    },
