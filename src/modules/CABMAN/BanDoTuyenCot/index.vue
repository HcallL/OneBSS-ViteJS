...
export default {
    ...
    txttimkiem: function (val) {
      try {
        ...
        if (val != '') {
          ...
          this.dsCotSearch = this.dsCotSearch.filter(
            x => x.SO_HIEU.toLowerCase().includes(val.toLowerCase()) || (x.COT_ID != undefined && x.COT_ID == 'COT_'+val)
          )
        }
      } catch (e) {
        this.dsCotSearch = [...this.dscot]
  ...
  methods: {
    ...
    lienketCot: async function (cot_id, cot_sau_id) {
      try {
        ...
        if (rs.error_code === 'BSS-00000000') {
          ...
          this.cotsau_id = null
        } else { this.$toast.error('Liên kết tới cột thất bại') }
      } catch (error) {
      } finally {
      }
    },
    ...
    xoaCot: async function () {
      try {
        if (this.cot_id_thao_tac != null) {
          ...
          } else { this.$toast.error('Xóa cột thất bại') }
        }
      } catch (error) {
    ...
    TaoDuLieuNode (cot, sau, i, count) {
      ...
      let limit = 5000
      this.demCot += 1
      if (this.demhang == 100) { this.demhang = 0 }

      let indx = this.demhang
      let col = parseInt(this.demCot / 100)
      console.log('col', col)
      if (!sau) {
        if (!this.hienThiGianDo) { node.offsetX = step }// 50;
        else { node.offsetX = limit }
      } else
      if (!this.hienThiGianDo) { node.offsetX = 100 * indx + 50 } else { node.offsetX = limit - step * indx }
      // if(i==0)
      // node.offsetY = 100
      // else
    ...
    luuToaDoBanDo: async function (nodes) {
      console.log('luuToaDoBanDo', nodes)
      let flag = true
      let message = []
      try {
    ...
    zoomIn: function () {
      if (!this.showcotChuaTD) { this.$refs.giando.ZoomIn() } else { this.setZoomMap(1) }
    },
    zoomOut: function () {
      if (!this.showcotChuaTD) { this.$refs.giando.ZoomOut() } else { this.setZoomMap(-1) }
    },
    zoomReset: function () {
      if (!this.showcotChuaTD) { this.$refs.giando.ZoomReset() } else { this.setZoomMap(0) }
    },
