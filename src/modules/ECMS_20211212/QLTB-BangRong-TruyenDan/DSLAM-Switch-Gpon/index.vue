...
export default {
    ...
    methods: {
        ...
        loadDsBras: async function() {
            try {
                var rs = await this.$root.context.get('/web-ecms/danhmuc/quanlybras/danhsach');
                this.dsBras = rs.data;
                // if(this.dsBras) {
        ...
        checkForm: async function() {
            ...
            var msg = '';
            if(this.kiemTraIp == 1) {
                var rs = await this.$root.context.get('/web-ecms/danhmuc/dongthietbi/kiemtra-trung-ip', {
            ...
            if(this.dslam.loaiTb_id == this.LOAI_TBI.SWITCH && !this.chk_subSlot) {
                if(this.dslam.soPort == null) {
                    check = false;
                    msg = 'Chưa nhập số port cho switch!\n'.concat(msg);
                }
                if(this.dslam.portBD == null) {
                    check = false;
                ...
                if(this.dslam_id) {
                    ...
                    if( rsDslam.data.length > 0) {
                        ...
                        this.dslam = {
                            ...
                            portBras : this.ttdslam.PORTBRAS_ID,
                            MDFBD : this.ttdslam.MDF_BD,
                            thongTinAE : this.ttdslam.THONGTIN_AE
                        }
                        if(this.dslam.subSlot) {
                            this.chk_subSlot = true;
                        }
                        this.chk_dothu = this.dslam.doThu ? true : false;
                        this.chk_doitac = this.dslam.doitac_id ? true : false;
                        this.chk_toanha = this.dslam.toanha_id ? true : false;
        ...
        setTuRack_id: async function(turack_id, loaiTb_id) {
            this.ttdslam = null;
            this.turack_id = turack_id;
            this.dslam.rack_id = [parseInt(turack_id)];
            this.dslam.loaiTb_id = loaiTb_id;
            this.dslam_id = null;
