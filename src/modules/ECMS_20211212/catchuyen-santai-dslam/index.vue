...
export default {
    ...
    methods: {
        setDisableBtnX: function (isHasData) {
                ...
                if (this.kiemtraDulieu())
                {
                    ...
                    if (dataIns != null) {
                        if (this.isCreateNew){
                            ...
                            if(rs){
                                this.$toast.error("DSLAM " + dataIns.dslam_id + " có bản ghi cắt chuyển chưa hoàn thành, do user " + this.message +" thực hiện !");
                                return;
                            }
                            else{
                            ...
                            this.catchuyen = {
                                ...
                                bras_id : this.currentRow.BRAS_ID,
                                dslam_cha_id : this.currentRow.DSLAM_CHA_ID,
            ...
            try {
                ...
                if (dataItem != null) {
                    ...
                    if (trangthai == 0)
                    {
                        ...
                        this.setDisableBtnX(false)
                    }
                    else
                    {
                        this.setDisableBtn(false, false, false, false);
                        this.setDisableBtnX(true)
        ...
        layDsCatChuyen: async function(){
            try {
                ...
                if (rs.data.length > 0) {
                    ...
                    this.setDisableBtnX(rs.data !== null && rs.data.length >0)
        ...
        laydulieuexcel: async function(dslam_id){
            this.dataExcel = []
            let rs = await this.$root.context.get('/web-ecms/catchuyendslam/laydulieuexceltheodslamid', { dslam_id: dslam_id })
            if (rs.data.length > 0) {
                let temp2 = rs.data;
