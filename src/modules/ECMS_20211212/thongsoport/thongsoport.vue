...
export default {
  ...
  methods: {
    getDonViQl: async function () {
      ...
      if (this.dataSelected.dslamTinh) {
        this.getVciChuaGan(this.dataSelected.dslamTinh, portId)
  ...
  addAllToDsDaGan: function() {
    ...
    addToDsChuaGan: function () {
      ...
      this.xoaVciVpiPort(this.dsVciDaGanSelected)
    },
    ...
    }, VciDaGanChange: function (dataItem) {
      if(dataItem){
    ...
    selectedPort: function (data) {
      ...
      if (dataItem) {
        ...
        if (dataItem.PORT_ID) {
          ...
          await this.getDsLoaiHinhDaGan(dataItem.PORT_ID)
        }
      ...
      try {
        ...
        this.$toast.error('Sinh Passonu thất bại.' + error)
        return '0'
      } finally {
        this.loading(false)
      }
    ...
    statusBtn(nhapMoi, ghiLai, huyBo, xoa) {
      ...
      this.$bvModal.msgBoxConfirm('Bạn thật sự muốn sinh Pass cho các port đã chọn không ?',
        ...
        if (value) {
          for (const value1 of this.dsPortSelected) {
            let ketqua = this.sinhPassOnu(value1)
            if (ketqua === '0') {
      ...
      this.$bvModal.msgBoxConfirm('Bạn thật sự muốn cập nhật trạng thái port ?',
        ...
        if (value) {
          this.updateTTChuaSD(this.dsPortSelected)

        }
      })

    }, btnCapNhatVcd: function () {
      if (this.dsPortSelected.length <= 0) {
        this.$toast.warning('Bạn chưa chọn port trong danh sách.')
        return
      }
      ...
      this.$bvModal.msgBoxConfirm('Bạn có muốn cập nhật danh sách port theo Dslam Visa: ' + this.dataSelected.tenDslamVdc,
        ...
        if (value) {
          this.updateVcdId(this.dsPortSelected)
