...
export default {
    ...
    methods: {
        ...
        onClickButtonGan () {
            ...
            try
            {
                ...
                selectedChuagan.forEach(item => {
                    ...
                    if (index !== -1) {
                    this.dsPortChuaGan.splice(index, 1)
                    this.dsPortDaGan.push(daGan[0])
                    }
                })
            }
            catch (ex)
        ...
        onClickButtonGoGan () {
            ...
            try
            {
                ...
                selectedDagan.forEach(item => {
                    ...
                    if (index !== -1) {
                    this.dsPortDaGan.splice(index, 1)
                    this.dsPortChuaGan.push(daGan[0])
                    }
                })
            }
            catch (ex)
        ...
        onChangeDsPortGanOrChua: async function(selectedIndex){
            try {
                this.loading(true)
                let kenhId = this.dataSelected.idNhom != null ? this.dataSelected.idNhom : -1
                let dslamTinh = this.dsTB
                let selectedDslam = dslamTinh[selectedIndex].DSLAM_ID
                if (selectedDslam === -1){
                    return
                }
