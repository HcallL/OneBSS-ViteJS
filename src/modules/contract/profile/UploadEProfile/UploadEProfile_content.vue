...
export default {
  ...
  mounted() {
    ...
    //   }
    // ];
    // this.fields={ dataSource: data, id: 'nodeId', text: 'nodeText', child: 'nodeChild' },
    // Split boxes
    try {
      if(!this.isCA) {
  ...
  data() {
    return {
      fields: { dataSource: [], id: "", text: "", child: "child" },
      groupByKey: {
        keyName: "Nhóm file",
        keyValue: "nhom_file",
      },
  ...
  methods: {
    ...
    ShowAlert(mesage, val) {
     ...
     changeDataSource (data) {
            let hoso_tree=this.createTreeDonVi(data)
            this.fields = {
                dataSource: hoso_tree, id: 'hoso_id', text: 'ma_hs_doituong', parentID: 'hoso_cha_id', hasChildren: 'hasChild'
            }
        },
        getFilterItems (fList, list) {
            let nodes = []
            nodes.push(fList['hoso_id'])
            let query2 = new Query().where('hoso_id', 'equal', fList['hoso_cha_id'], false)
            let fList1 = new DataManager(list).executeLocal(query2)
            if (fList1.length !== 0) {
                let pNode = this.getFilterItems(fList1[0], list)
                for (const element of pNode) {
                    if (nodes.indexOf(element) === -1 && element != null) {
                        nodes.push(element)
                    }
                }
                return nodes
     ...
     searchHoso () {
            ...
            if (this.txtDoiTuong.trim() !== '') {
                ...
                predicate = new Predicate('ma_hs_doituong', 'contains', this.txtDoiTuong.trim(), true)
                
                let filteredList = new DataManager(this.ds_hoso).executeLocal(new Query().where(predicate))
                
                for (const element of filteredList) {
                    _filter.push(element['hoso_id'])
                    let filters = this.getFilterItems(element, this.ds_hoso)
                    for (const element of filters) {
                        if (_array.indexOf(element) === -1 && element != null) {
                            _array.push(element)
                            predicats.push(new Predicate('hoso_id', 'equal', element, false))
                        }
                    }
                }
                if (predicats.length === 0) {
                    this.changeDataSource([])
                    this.$toast.error("Không tìm thấy hồ sơ")
                } else {
                    let query = new Query().where(Predicate.or(predicats))
                    let newList = new DataManager(this.ds_hoso).executeLocal(query)
                ...
                this.changeDataSource(this.ds_hoso)
            }
        },
    createTreeDonVi(dsHoso){
            let tree = []
            let hosoChaChaArray = dsHoso.map((item) => {
                return item['hoso_id']
            })
            let hosoArray = dsHoso.map(item => (item['hoso_id']))
            dsHoso.forEach(item => {
                if(hosoChaChaArray.includes(item['hoso_id'])){
                    if(!hosoArray.includes(item['hoso_cha_id'])){
      ...
      UploadEProfileAPI.get_danhsach_doituong_hoso(this.axios, postData).then(
        (response) => {
          ...
          if (
            ...
            this.listbox.doituong_hoso.list = response.data.data;
            this.ds_hoso = response.data.data;
            let hoso_tree=this.createTreeDonVi(this.ds_hoso)
            this.fields = {
                dataSource: hoso_tree,
                id: 'hoso_id',
                text: 'ma_hs_doituong',
    ...
    addNodes(node) {
      ...
      var nodes_data = this.$refs["treeview"].getTreeData(node.nodeData.id);
     
      if (node.nodeData.id == -1) {
        return;
      }  
      var item = nodes_data[0];
      if (node.nodeData.hasChildren && item.child.length <= 0) {
        UploadEProfileAPI.danhsach_node_thietbi_con(this.axios, {
          hoso_id: node.nodeData.id
        }).then(response => {
          response.data.data.forEach(i => {
            i.has_child = i.has_child != 0 ? true : false;
            i.child = [];
          });
          item.child = this.sortTree(response.data.data);
          ...
          this.$refs["treeview"].expandAll([node.nodeData.id], 1, false);
        });
      }
    },
    sortTree() {
    ...
    treeClicked(data) {
      ...
      var node = {};
      if (!isNaN(data)) {
        node.nodeData = this.$refs["treeview"].getNode(data);
      } else {
        node = data;
      }
      this.listbox.doituong_hoso.value = node.nodeData.id;
      if (this.$refs["searchEProfile-modal"].isShow == true) {
        this.$bvModal.hide("searchEProfile-modal");
        UploadEProfileAPI.get_ds_file_hs_ct(this.axios, {
          hoso_id: node.nodeData.id,
          file_id: this.listbox.ds_file_hs.value.file_id
        }).then(response => {
          this.loading(false);
          if (
            response.data.error_code &&
        ...
        UploadEProfileAPI.get_ds_file_hs(this.axios, {
          hoso_id: node.nodeData.id
        }).then(response => {
          this.loading(false);
          if (
    ...
    fileHSChanged(obj) {
      ...
      console.log(this.listbox.ds_file_hs.value)
    ...
    btnLayTT() {
      this.refreshTree(); // focus root node
    },
    btnRefresh() {
    ...
    btnNewClicked() {
      var nhomfile_id = this.listbox.ds_file_hs.value.nhomfile_id
      var loaifile_id = this.listbox.ds_file_hs.value.loaifile_id
      var kieu = this.listbox.ds_file_hs.value.kieu
      this.listbox.ds_file_hs.value = { 
        nhomfile_id: nhomfile_id,
    ...
    async btnUpdateClicked() {
      ...
      if (
        ...
        var txt = !this.listbox.ds_file_hs.value.loaifile_id ? 'Loại file' :
                  !this.listbox.ds_file_hs.value.kieu ? 'Kiểu file' :
                  !this.listbox.ds_file_hs.value.url ? 'Đường dẫn' : 'Không xác định'
        this.$toast.error('Thông tin File không được bỏ trống: '+txt+' !')
        this.loading(false);
        return;
      }
      if (this.isAddNew) {
        ...
        if (this.listbox.ds_file_hs.value.nguoi_cn != this.tt_nd.ma_nd) {
    ...
    btnDeleteClicked() {
      ...
      if (!ds_file_hs || ds_file_hs.length == 0) {
      ...
      this.$bvModal
        ...
        .then(async (v) => {
          if (v) {
            ...
            // xoá 1
            if (ds_file_hs.length == 1) {
              if (this.tt_nd.ma_nd != this.listbox.ds_file_hs.value.nguoi_cn) {
                if (this.tt_nd.ma_nd != "admin") {
                  this.ShowAlert(
                    "Bạn không được quyền sửa xóa file hồ sờ của người dùng khác",
                    1
    ...
    async UploadFileHoSo() {
      ...
      if (this.gioihan_loaifile == 1) {
        ...
        UploadEProfileAPI.kiemtra_loaifile_duoc_up(this.axios, {
          vfile_ext: vfile_ext,
          vloaitb_id: this.vloaitb_id
        }).then(response => {
          if (response.data.error_code != this.success_code) {
            this.ShowAlert(response.data.error_code, 1);
            return;
          }
        });
      }
    ...
    async FIND_NODE_FILE(item) {
      if (item.isfile == 1 ) {
        this.$refs["treeview"].selectedNodes = [item.hoso_id];
        this.listbox.ds_file_hs.value.file_id = item.id
      }
    },
