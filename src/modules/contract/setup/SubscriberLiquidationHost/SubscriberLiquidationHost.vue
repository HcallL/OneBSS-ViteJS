<template src="./SubscriberLiquidationHost.html"></template>
<script>
...
import DichVuVienThong from './DichVuVienThong.json'
import TRANGTHAI_DONGBO from './TRANGTHAI_DONGBO.json'
import TrangThaiHD from './TrangThaiHD.json'
import LoaiHinhTB from './LoaiHinhTB.json'
...
import NHOM_KQXL from './NHOM_KQXL.json'
...
import DONGBO_GPHONE from './DONGBO_GPHONE.json'
import InBienBan from '@/modules/print/InBienBan/InBienBan.vue'
import DangNhapIVAN from './DangNhapIVAN.vue'
...
export default {
  ...
  async mounted () {
    ...
    this.setTimeout(() => {
      this.setVisibleButton('tsbtnGiaoPhieu', true)
  ...
  methods: {
    ...
    getTag () {
      try {
        ...
        } else if (this._Tag != null && this._Tag != "" && this._Tag.length > 0) {
          console.log("Chay vao form nay tu form khac: " + this._Tag + " ||| " + this.matb_link + " ||| " + this._vquytrinh_id)
          let tag = decodeURIComponent(this._Tag)
          let arr = tag.split('+')
          if (arr.length >= 1) {
            this.tthd_id = Number.parseInt(arr[0])
          }
    ...
    async cboDichVuVT_SelectedValueChanged() {
      this.dichvuvt_id = Number.parseInt(this.listbox.cboDichVuVT.value)
        ...
        await API.sp_lay_ds_quytrinh_v2(this.axios, data).then(response => {
          ...
          if (this.listbox.cboQuyTrinh.list.length > 0) {
            ...
            if (this._vquytrinh_id && this._vquytrinh_id != 0) {
                this.listbox.cboQuyTrinh.value = this._vquytrinh_id
            }
          }
        })
      } else {
         ...
         * NewLife
         */
        let dt_quytrinh = (await API.lay_ds_quytrinh_newlife(this.axios, {
          "vloaihd_id": LoaiHopDong.CHAMDUT_SD,
          "vdichvuvt_id": DichVuVienThong.ADSL,
          "vtthd_id": this.tthd_id
        })).data.data
        if(dt_quytrinh.length > 0) {
          this.listbox.cboQuyTrinh.list = dt_quytrinh.map(x => {
            return {id: x['quytrinh_id'], text: x['quytrinh']}
          })
          this.listbox.cboQuyTrinh.value = this.listbox.cboQuyTrinh.list[0].id
          if (this._vquytrinh_id && this._vquytrinh_id != 0) {
      ...
      try {
        ...
        if (this.ktra_load === true) {
          ...
          if(!this.cty_newline) {
            ...
            data = this.GetDataList(await API.sp_lay_ds_thuebao_thanhly(this.axios, req))
          } else {
            data = (await API.sp_lay_ds_thuebao_thanhly_newlife(this.axios, {
              "p_phanvung_id": null,
              "p_dichvuvt_id": this.listbox.cboDichVuVT.value,
              "p_bancheo": 0,
              "p_lay_phieu_con": this.lay_phieu_con ? 1 : 0,
              "p_kieu": this.phieu_moi,
              "p_tthd_id": this.tthd_id,
              "p_huonggiao_id": this.huonggiao_id,
              "p_nhanvien_id": this.nhanvienId,
              "p_loai_id": this.phieu_moi,
              "p_ds_chuquan_theo_quyen": this.DS_CHUQUAN_THEO_QUYEN ? `(${this.DS_CHUQUAN_THEO_QUYEN})` : ''
            })).data.data
          }
          if (data.length > 0) {
            data.forEach((item) => {
      ...
      try {
        ...
        if (item != null) {
          ...
          if (this.dichvuvt_id === DichVuVienThong.CO_DINH || this.dichvuvt_id === DichVuVienThong.ADSL ||
            ...
            } else if (this.dichvuvt_id === DichVuVienThong.TSL || this.dichvuvt_id === DichVuVienThong.TSL_DOITAC || this.dichvuvt_id == 25) {
          ...
          if (this.PHAILAM('KICHHOAT_TD')) {
            if (item.ngay_ht_gp) {
              ...
              if (this.dichvuvt_id === DichVuVienThong.ADSL || this.dichvuvt_id === DichVuVienThong.IMS ||
                ...
                this.dichvuvt_id === DichVuVienThong.GPHONE || this.dichvuvt_id == 25 || this.dichvuvt_id == 26) {
    ...
    tsbtnChapNhan_Click: async function () {
      ...
      if (this.formProps.txtNoiDungTH != undefined && this.formProps.txtNoiDungTH != null && this.formProps.txtNoiDungTH.length > 4000) {
        this.$toast.error('Nội dung TH không được vượt quá 1000 ký tự')
        $("#txtNoiDungTH").focus()
        return
      }
      ...
      if (this.gview_DanhSach.list.length > 0) {
          ...
          if (index >= 0) {
              ...
              if (this.matb_link && this.matb_link != "") {
                  let index2 = this.gview_DanhSach.list.findIndex(item => item.ma_tb === this.matb_link)
                  if (index2 >= 0) {
                      this.$refs.dsphieu.$refs.grid.selectRow(index2)
                  } else {
          ...
          for (let i = 0; i < this.gview_DanhSach.checked.length; i++) {
            ...
            if (this.dichvuvt_id === DichVuVienThong.ADSL || this.dichvuvt_id === DichVuVienThong.IMS) {
              if (Number(item.loaitb_id) !== LoaiHinhTB.INTERNET_ADSL_TINH_KHAC &&
                ...
                Number(item.loaitb_id) !== LoaiHinhTB.WIFI_MESH) {
                if (Number(item.status) !== TRANGTHAI_DONGBO.DONGBO_CM) {
    ...
    async btnTraPhieu_Click () {
      ...
      // this.modelTraPhieu.nhanvienId = 
      // this.modelTraPhieu.ma_tb = 
    ...
    async tsbtnHoanThanh_Click () {
      try {
        ...
        if (this.dichvuvt_id == DichVuVienThong.TSL || this.dichvuvt_id == DichVuVienThong.TSL_DOITAC || this.dichvuvt_id == 25) {
          if (this.gview_DanhSach.selectedItem === null) {
                ...
                if (ds.length > 0 && ds[0].hdtb_ban && Number(ds[0].hdtb_ban) > 0) { // bên thi công hoàn công update dữ liệu bên bán
                    ...
                    if (ds2.length != 0) { //anhnt thêm điều kiện thuebao_id_thicong != -1 cho chỉ giao net
                        if (String(ds2[0].loaihd_id) == String(LoaiHopDong.DAT_MOI) && 
                            ...
                            if (String(ds2[0].dichvuvt_id) != String(DichVuVienThong.TSL) && String(ds2[0].dichvuvt_id) != "25") {
