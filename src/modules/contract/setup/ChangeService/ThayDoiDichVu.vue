...
export default {
  ...
  async created() {
    ...
    if (ds_tsmd && ds_tsmd.length>0) {
      ...
      this.ts_sinhma_gd_theo_donvi = ds_tsmd.filter((item) => item.ma_ts == 'SINH_MA_GD_THEO_DV').length > 0
    }
  ...
  methods: {
    ...
    async HienThiTTHopDongKH(ds){
      try
      {
          ...
          if (ds.length > 0)
          {
              ...
              this.HienThiTTHopDongTB(res);
              this.HienThiDanhSacHDTB(this.hdkh_id);
    ...
    async HienThiTTDBMoRong(thuebao_id, dichvuvt_id){
      ...
      else if (parseInt(this.selectedDichVu) == DICHVU_VIENTHONG.MEGAWAN)
      {
      ...
      else if (parseInt(this.selectedDichVu) == DICHVU_VIENTHONG.TSL)
      {
    ...
    async KiemTraDL_ThueBao() {
      ...
      for (let i=0;i<this.arrayItemDaDangKy.length;i++){
        ...
        if (row.dangky==1) {
          ...
          } else if (this.selectedDichVu.toString() === DICHVU_VIENTHONG.TSL.toString())
          {
    ...
    async tsbtnThemTB_Click() {
      ...
      try {
        ...
        const ds_isdn = this.TaoDuLieu_HDTB_ISDN(dset);
        const dgvDVGT = this.TaoDuLieu_DVGT();
        const HDKH = await this.TaoDuLieu_HDKH();
        const HDTB = await this.TaoDuLieu_HDTB(dset);
    ...
    async CapNhat(){
      ...
      if (this.themMoi === 1) {
        ...
        // lấy mã giao dịch theo cách mới
        if (this.ts_sinhma_gd_theo_donvi) {          
          let res = await api.SINH_MAGD_V2({
            loaihd_id: 7,
            donvi_id: parseInt(this.$root.token.getDonViID())
          })
          if (res && res.data && res.data.data) {
            this.maGiaoDich = res.data.data
          }
        } else {          
          let res = await api.SINH_MAGD({ loaihd_id: 7 })
          if (res && res.data && res.data.data) {
            this.maGiaoDich = res.data.data
          }
        }
      ...
      if (this.arrayItemDaDangKy.length>0) {
        ...
        try {
          const inputHuongTiepNhan = {
            loaiHdId: LOAI_HOPDONG.THAY_DOI_DV,
            loaiTbId: this.loaitb_id,//parseInt(this.selectedLoaiHinh),
            tocDoId: this.tocdotn_id,
            mucCuocId: this.muccuoctn_id,
            kieuLdId: this.selectedKieuYeuCau
          };
          ...
          const dgvDVGT = this.TaoDuLieu_DVGT();
          const HDKH = await this.TaoDuLieu_HDKH();
          const HDTB = await this.TaoDuLieu_HDTB(dsHuongTiepNhan);
          const HDTB_DV = await this.TaoDuLieu_HDTBDV();
          const dangky_dvgt = this.TaoDuLieu_HDTDDV();
          const CTTHD = await this.TaoDuLieu_CT_TIENHD();
          const HDTB_CD = await this.TaoDuLieu_HDTB_CD();
          ...
          if (status.MESSAGE === "Thành công") {
            ...
            // this.loadDataDichVuGTByThueBao();
          } else {
            this.$toast.error(`${status.MESSAGE}`);
            this.maGiaoDich =''
            this.hdtb_id=0
    ...
    async HienThiTTHopDongTB(ds){
      ...
      if (this.selectedDichVu == DICHVU_VIENTHONG.ADSL)
      {
          ...
          if (ds_con && ds_con.length > 0) {
            ...
            if(ds_con.tocdo_id){
              this.tocdotn_id = parseInt(ds_con.tocdo_id)
              if (this.selectedLoaiHinh!=LOAIHINH_THUEBAO.INTERNET_MYTV && this.selectedLoaiHinh!=LOAIHINH_THUEBAO.INTERNET_MYTV_B2B && this.selectedLoaiHinh!=LOAIHINH_THUEBAO.INTERNET_SGTV
                  && this.selectedLoaiHinh!=LOAIHINH_THUEBAO.HTVC_FIBERVNN_ANALOG && this.selectedLoaiHinh!=LOAIHINH_THUEBAO.HTVC_FIBERVNN_DIGITAL){
                    if (ds_con.muccuoc_id)
                      this.muccuoctn_id = parseInt(ds_con.muccuoc_id)
      ...
      } else if (this.selectedDichVu == DICHVU_VIENTHONG.MEGAWAN)
      {
          ...
          if (ds_con && ds_con.length > 0) {
              this.maHDTTKD = ds_con.ma_pl
              this.soAo = ds_con.ma_lt
              this.huongKetNoi = ds_con.huong_kn
              if (ds_con.tocdo_id)
                  this.tocdotn_id = parseInt(ds_con.tocdo_id)
              this.muccuoctn_id = 0;

              if (ds_con.kp_ao.toString() == "1")
                  this.kt_kenhphu_ao = true
              else
                  this.kt_kenhphu_ao = false

              this.maBSS = ds_con.ma_bss
              this.maERP = ds_con.id_erp
          }
                    ...
                    || this.selectedDichVu == DICHVU_VIENTHONG.DICHVU_CNTT || this.selectedDichVu == DICHVU_VIENTHONG.HOINGHI_TRUYENHINH) //Dich vu CNTT
      {
          ...
          if (ds_con && ds_con.length > 0) {
              if (ds_con.linhvuc_id)
                  this.linhvuc_id = parseInt(ds_con.linhvuc_id)
              else
                  this.linhvuc_id = 0
              this.maHDTTKD = ds_con.ma_pl
          }
          ...
          {
              if (ds_con.tongso_nd)
                  this._sl_cha = parseInt(ds_con.tongso_nd)
              else
                  this._sl_cha = 0
          }
          if (parseInt(this.selectedLoaiHinh) == LOAIHINH_THUEBAO.HNTH_NGN
              || parseInt(this.selectedLoaiHinh) == LOAIHINH_THUEBAO.VNPT_MEETING)
          {
              if (ds_con.sl_mailing)
                  this._sl_cha = parseInt(ds_con.sl_mailing)
              else
                  this._sl_cha = 0
          }
      }
    ...
    async TaoDuLieu_HDTB_MGWAN() {
      if (parseInt(this.selectedDichVu) != DICHVU_VIENTHONG.MEGAWAN && parseInt(this.selectedDichVu) != DICHVU_VIENTHONG.METRONET) return []
      try {
    ...
    async TaoDuLieu_HDTB_TSL() {
      ...
      let obj = {};
      let dsHDTB_TSL=[]
      if (parseInt(this.selectedDichVu) != DICHVU_VIENTHONG.TSL) return []
