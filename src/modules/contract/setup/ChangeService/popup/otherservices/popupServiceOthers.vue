...
export default {
    ...
    methods: {
      ...
      closeForm(){
        if (this.dbDichVu && this.dbDichVu.length>0) {
      ...
      GetDsDVKhac : async function(){
        let rs = await this.$root.context.post('/web-hopdong/thaydoithongtinkhuyenmai/LAY_DS_DICHVU_KHAC_DBID', this.dulieu);
        console.log(rs.data)
        if(rs.data){
