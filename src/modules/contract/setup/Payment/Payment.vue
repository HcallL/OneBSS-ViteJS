...
export default {
  ...
  data() {
    return {
      ...
      HinhThucTra: {
        ...
        UY_NHIEM_CHI: 4,
        UY_NHIEM_THU: 5,
        VI_VNPTPAY: import.meta.env.NODE_ENV == 'testing' ? 204 : 7,
        TT_CHUOITOANQUOC: 8,
        TT_SHOP_ONLINE: 9,
        KHONG_THU_TIEN: 208,
      },
      KENHTHU: {
        TAI_GIAO_DICH: import.meta.env.NODE_ENV == 'testing' ? 22 : 1,
        TAI_NHA: import.meta.env.NODE_ENV == 'testing' ? 21 : 2,
        BUU_CUC: 3,
        BAO_LANH: 4,
        CHUYEN_KHOAN: 5,
        KHONG_THU_TIEN: 6,
  ...
  async created() {
    ...
    try {
      Promise.all([this.getDmTrangThaiTT(), this.getDmHinhThucTra(), this.getDmNganHang(), this.getDmLoaiHoaDon(), this.getDmSeri(), this.getDmLoaiTien(), this.getDmThuNgan(), this.getDsLoaiHD(), this.getDsDVTT(), this.getDsTTHD(), this.getDongBoVina()])
        .then(async (values) => {
      ...
      if (defaultVals && defaultVals.length > 0) {
        ...
        if (defaultVals.filter((x) => x.ma_ts == 'OPEN_SYNC_VINA').length > 0) {
          ...
          console.log('🚀 ~ file: Payment.vue:1300 ~ created ~ valObj:OPEN_SYNC_VINA', valObj)
          if (valObj.ten_ts == 1) {
        ...
        if (defaultVals.filter((x) => x.ma_ts == 'OPEN_SYNC_VINA_TRASAU').length > 0) {
          ...
          console.log('🚀 ~ file: Payment.vue:1300 ~ created ~ valObj:OPEN_SYNC_VINA_TRASAU', valObj)
          if (valObj.ten_ts == 1) {
      ...
      try {
        ...
        if (!this.cty_newline) {
          if (import.meta.env.NODE_ENV == 'testing') {
            input = { nghiepvu_id: this.KieuId == 1 ? 1 : 2, ht_tra_id: this.hinhThucTTId }
        ...
        this.DmKenhThu = data?.map((x) => ({
          ...
          trangthai: x?.trangthai || x?.TRANGTHAI || 0,
        })) || []
      } catch (e) {
    ...
    getDsHopDongByMaGD: async function() {
      ...
      if (data != null && data.length > 0) {
        ...
        if (this.KieuId == 1) {
          if (!(await this.kiemTraKenhTiepNhan(this.HdKhId))) {
            ...
            if (kt.length > 0) {
              this.trangThaiThanhToanId = kt[0]?.trangthai || 0
            }
          } else {
      ...
      if (this.KieuId == 2 || this.KieuId == 3) {
        ...
        if (ds_phieutt && ds_phieutt.length > 0) {
          ...
          if (parseInt(this.trangThaiThanhToanId) == 1 || parseInt(this.trangThaiThanhToanId) == 2) {
            this.Fn.GachNo = false
            this.Fn.InHD = true
    ...
    KiemTraDuLieu: async function() {
      ...
      if (this.$v.$invalid) {
        ...
        if (this.chkLoaiHDStatus && this.chkLoaiHD) {
          ...
          //   errorMessage.push(`Bạn chưa nhập số phiếu thu !`);
        ...
        if (import.meta.env.NODE_ENV == 'testing') {
          ...
          if (this.KieuId == 1 && this.LoaiHDId == 700 && val == this.HinhThucTra.TIEN_MAT) {
        ...
        // {
        //   errorMessage.push("Hóa đơn khách hàng có khoản mục khác 0. Bạn không được chọn kênh thu 'Không thu tiền' !");
        //     // return false;
        // }
        //     if (this.tag == 1 && this.ts_canhbao_phathanh_hddt == 1 && this.chkHoaDonDienTu == false)
        //     {
        //         if (!confirm("Bạn chưa chọn phát hành hóa đơn điện tử. Bạn muốn tiếp tục thực hiện thanh toán ?"))  {
        //           errorMessage.push('Chưa phát hành hóa đơn điện tử. Vui lòng kiểm tra')
        //         };
        //     }
        // }
        // else
        ...
        //     {
        //       errorMessage.push("Hóa đơn khách hàng có tổng tiền = 0. Bạn không được chọn kênh thu khác 'Không thu tiền' !");
        //       // return false;
        //     }
        // }
        ...
        if (this.kenhThuId == this.KENHTHU.BAO_LANH || this.kenhThuId == this.KENHTHU.CHUYEN_KHOAN) {
          //
          if (this.txtNguoiBL == '') {
            errorMessage.push(`Bạn phải chọn tên người bảo lãnh khi chọn kênh thu: Bảo lãnh hoặc Chuyển khoản !`)
            try {
        ...
        if (phanvungId == '21') {
          if (this.kenhThuId == this.KENHTHU.TAI_NHA) {
            ...
            if (!result) errorMessage.push(`Bạn không được chọn kênh thu: Nộp tiền sau !`)
          }
        }
        // Kiem tra ngan hang
    ...
    LayDanhSachTBTT_DiDong: function() {
      ...
      for (const item of self.DsThueBao.data) {
        ...
        if (
          ...
          item.kieuld_id != self.KieuLapDat.DM_DIDONG_CHUYEN_TINH && //Không lấy thuê bao dd hòa mạng chuyển tỉnh
          item.status == '1'
        ) {
    ...
    async kenhThuIdChanged(val) {
      ...
      try {
        ...
        try {
          ...
          } else this.trangThaiThanhToanId = 0
        } catch (ex) {}
        ...
        if (this.KieuId == 1) {
          if (import.meta.env.NODE_ENV != 'testing') {
            ...
            } else if (this.hinhThucTTId == this.HinhThucTra.KHONG_THU_TIEN) {
