...
export default {
  ...
  mounted: async function() {
    ...
    if (this.$route.query.thutu_id && this.$route.query.thutu_id !="")
    {
      //this.isTimTheoQuyTrinh = true;
      this.thutu_id =  this.$route.query.thutu_id;
    }
  ...
  watch: {
    // 'listbox.loaihd.value.LOAIHD_ID': function(newVal, oldVal) {
    // }
  },
  data () {
    return {
          ...
          if(response.data.error_code && response.data.error_code === this.success_code) {
            ...
            if (this.loaidv_nhan_id != "-1")
            {
              ...
              API.sp_ht_tatca_vetinh_combobox_v2(this.axios, input1).then((response) => {
                if(response.data.error_code && response.data.error_code === this.success_code) {
                  this.listbox.donvinhan.list = response.data.data.map(x=>({id:x.donvi_id,text:x.ten_dv})).filter(x=>x.text!=null);
    ...
    btnGiaoPhieu()
    {
     ...
     /*  console.log(moment(this.ngaygiao).format("DD/MM/YYYY"),moment(currentdate).format("DD/MM/YYYY"));
      ...
      try{
        ...
        this.ShowAlert("Lỗi lấy danh sách loại hợp đồng",2);this.loading(false); }
      ...
      for (var item of listphieuchuyen)
      {
        if (this.loaidv_nhan_id != "-1")
        {
          try{
            ...
            input1.loaidv_id = this.loaidv_nhan_id;
            if (this.listbox.dichvu.value.DICHVUVT_ID != this.DV_BTSL && this.listbox.dichvu.value.DICHVUVT_ID != 25)//nếu là dịch vụ TSL
            {
              input1.kieudv_id =2;
            }
            else
