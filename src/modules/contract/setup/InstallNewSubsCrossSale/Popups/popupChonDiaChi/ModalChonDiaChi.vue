...
export default {
  ...
  methods: {
      ...
      if(this.dsTinh && this.dsTinh.length>0 && (!this.diachi.tinh_id || this.diachi.tinh_id==0))
        this.diachi.tinh_id = this.$root.token.getPhanVungID();//this.dsTinh[0].tinh_id;
      if(this.$root.token.getPhanVungID()<96 || this.$root.token.getPhanVungID()>100) {
        this.dsTinh = this.dsTinh.filter(x=>x.tinh_id==this.diachi.tinh_id || x.tinh_id==this.$root.token.getPhanVungID() || x.tinh_id==(this.tinhbancheo?this.tinhbancheo:-1));
    ...
    getQuan: async function () {
      try {
        console.log('?????',this.dsTinh,this.diachi.tinh_id);
        this.$root.showLoading(true);
