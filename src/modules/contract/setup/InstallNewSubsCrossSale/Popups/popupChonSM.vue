...
<div class="modal-content popup-box">
    <div class="popup-header">
        <div class="title"><span class="icon one-notepad"></span>Chọn số máy BC</div>
...
export default {
    ...
    async mounted() {
        ...
        {
            console.log('mounted')
            console.log(this.dulieu._tinh_id)
            ...
            this.mavung = this.dulieu._mavung;
    ...
    methods: {
        ...
        getDauSo: async function() {
            ...
            try {
                ...
                var dsDauSo = await this.$root.context.post("/web-bancheo/tracuu/lay_ds_dauso_theo_quan_phuong",{"tinhthicong_id":this.diachi.tinh_id, "vdichvuvt_id": this.dichvuvt_id, "vdonvi_ql_id": 0, "vloaiso_id": this.loaiso.value, "vloaitb_id": this.loaitb_id, "vnhomso_id": this.nhomso.value, "vphuong_id": this.diachi.phuong_id, "vquan_id": this.diachi.quan_id, "vvetinh_id": this.vetinh.value }); //loaitb = 0 do bien mac dinh
                this.loading(false);
        ...
        chapnhan: function () {
            ...
            //     return;
            // }
            let somay = this.somay.value
            if(this.mavung) {
                let dauso=''
                if (this.mavung.substring(0,2)==='84'){
                    dauso = "0" + this.mavung.substring(2);
                    
                } 
                else dauso = this.mavung
                somay = dauso + this.somay.value

                if(this.somay.valuearr && this.somay.valuearr.length>0)
                    for(var i=0;i<this.somay.valuearr.length;i++) {
                        this.somay.valuearr[i] = dauso + this.somay.valuearr[i];
                    }
            }
            var retData = {somay:somay,loaiso:this.loaiso.value,arraySoMay:this.somay.valuearr,nhomso:this.nhomso.value,tiensodep:this.tiensodep};
            if(this.thueso) {
        ...
        async dsDauSoChanged(idx,item) {
            if(!this.ds_dauso || this.ds_dauso.length<=0) {
                ...
                try {
                    ...
                    var rs = await this.$root.context.post("/web-bancheo/tracuu/lay_ds_daiso_cd_ims_gp", {
                    ...
                    this.loading(false);
                    this.$toast.error('Có lỗi lấy danh sách dải số!');
                }
