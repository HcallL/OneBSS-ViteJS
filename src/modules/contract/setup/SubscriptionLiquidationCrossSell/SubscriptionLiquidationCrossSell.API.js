...
export default {
  ...
  layhuonggiao_tiepnhan_bc: (tinhthicong_id, loaihd_id, loaitb_id, tocdo_id, muccuoc_id, kieuld_id) => axios.post(`web-bancheo/danhmuc/layhuonggiao_tiepnhan`, {
  ...
  lay_thuebaoid_thicong: (vtinh_id, vthuebao_id) => axios.post(`web-hopdong/chuyendoi-codinh-sang-ims-bancheo/lay_thuebaoid_thicong`, {
    ...
    return rs.data.data[0].thuebao_kn_id
  }),
  ...
  themmoi_HDBANCHEO_BIENDONG_V2: (dsHDBC, dsHDTBBC, dsHDBC_MGW, dsHDBC_ADSL, dsHDBC_CD, dsDK_DVGT, dsHDBC_TSL, dsHDBC_IMS, dsDKHUYBC, dsBD_LYDOHUY,huonggiao_tn) => {
    let p_ds_para = JSON.stringify( {
      "huonggiao_id": huonggiao_tn,
      "js_hdBANCHEO": dsHDBC,
      "js_hdtbBANCHEO": dsHDTBBC,
      "js_dsHDBCWAN": dsHDBC_MGW,
      "js_dsHDBCADSL": dsHDBC_ADSL,
      "js_dsHDBCCD": dsHDBC_CD,
      "js_dsHDBCDVGT": dsDK_DVGT,
      "js_dsHDBCTSL": dsHDBC_TSL,
      "js_dsHDBCIMS": dsHDBC_IMS,
      "js_dsLYDOHUYBC": dsDKHUYBC,
      "js_dsBDLYDOHUYBC": dsBD_LYDOHUY
    })
    return axios.post(`web-bancheo/thaydoidvgtbc/fn_themmoi_hdbancheo_biendong_v2`, {'p_ds_para': p_ds_para}).then(rs => rs.data)
  },
  ...
  kiemtra_tb_cung_makh_tb_pl: (maTb, thuebaoId) => axios.post(`web-hopdong/chuyendich/kiemtra_tb_cung_makh_tb_pl`, {
    "maTb": maTb,
    "thuebaoId": thuebaoId
  }).then(rs => rs.data.data),
  kiemtra_hdtb_khi_themmoi: (data) => axios.post(`/web-hopdong/hopdong/kiemtra_hdtb_khi_themmoi`, data).then(rs => rs.data.data),
  ...
  lay_ds_kieu_lapdat: (loaitb_id) => axios.get(`web-hopdong/thanhly-thuebao-bancheo/sp_ht_kieu_ld_thanhly_tinhkhac?loaitb_id=${loaitb_id}`).then(rs => rs.data.data),


  // thoains-fix ATT
  lay_dulieu_theo_dieukien_toc_do_kenh: () => axios.post(`web-hopdong/laydulieu/sp_tt_tocdo_kenh`,{ p_type: 2 } ).then(rs => rs.data.data),

  lay_dulieu_theo_dieukien_tocdo_adsl :(loaitbID)=> axios.post(`web-hopdong/laydulieu/sp_tt_tocdo_kenh`, { p_type : 5, p_param : loaitbID }
 ).then(rs => rs.data.data ),
 fn_tt_muccuoc_tb: (p_phanvung,muccuoctb_id) => axios.post(`web-tratruoc/thongtin_tratruoc_dn/fn_tt_muccuoc_tb`, {
  "p_phanvung":p_phanvung,
  "type": 1,
  "param": muccuoctb_id
 }).then(rs => rs.data.data),
 ...
 fn_tt_doitac: async (axios, data) => axios.post('/web-tratruoc/thongtin_tratruoc_dn/fn_tt_doitac', data),
