...
export default {
  ...
  async created() {
    ...
    try {
      ...
      if (words.length >= 1)
          this.tag = words[0]
      if (words.length >= 2)// Áp dụng cho user của đối tác
      {
        if (words[1] == "1")
  ...
  methods: {
      ...
      if (this.$v.$invalid) {
        ...
        if (this.chkHoaDonDienTu && this.chkHoaDonDienTuVisible) {
          if (this.kenhThuId == this.KENHTHU.KHONG_THU_TIEN) {
            errorMessage.push(`Không được xuất HĐĐT với kênh thu này ! `)
          }
          if (this.txtMaSoThue == '') {
        ...
        if (this.kenhThuId != this.KENHTHU.KHONG_THU_TIEN) {
          if (parseInt(this.txtTongTienTra) == 0) {
            // errorMessage.push(`Hóa đơn khách hàng tổng tiền = 0 . Bạn không được chọn kênh thu khác 'Không thu tiền' !`)
          }
        }
    ...
    SwitchPayment: async function(data) {
      ...
      } else if (data.result == '0' && detail.MESSAGE.indexOf('Bạn muốn tiếp tục thực hiện thanh toán') >= 0) {
        this.$bvModal
      ...
      } else if (data.result == '0' && detail.MESSAGE.indexOf('Bạn có muốn thanh toán cho các thuê bao này không') >= 0) {
        let phanvungId = this.$root.token.getPhanVungID()
        this.$bvModal
          .msgBoxConfirm('Bạn có muốn thanh toán cho các thuê bao này không?', {
            title: '',
      ...
      } else if (data.result == '0' && detail.MESSAGE.indexOf('Hợp đồng chưa có thông tin liên hệ, bạn phải bổ sung thông tin liên hệ trước khi thanh toán') >= 0) {
        this.$bvModal
