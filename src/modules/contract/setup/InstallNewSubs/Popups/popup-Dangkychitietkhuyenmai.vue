...
import { DichVuVienThong } from '../Enums'
...
export default {
  ...
  data() {
    return {
      ...
      cols: [
        ...
        { fieldName: 'datcoc_csd', headerText: 'Tiền trả trước', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'tyle_sd', headerText: 'Tỷ lệ SD', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'tyle_tb', headerText: 'Tỷ lệ TB', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'tien_sd', headerText: 'Tiền SD', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'tien_tb', headerText: 'Tiền TB', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'cuoc_sd', headerText: 'Giảm cước SD', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'cuoc_tb', headerText: 'Giảm cước TB', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        { fieldName: 'tyle_km', headerText: 'Tỷ lệ KMLĐ', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
        ...
        { fieldName: 'tyle_vat', headerText: 'Tỷ lệ VAT', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: false }
      ],
      dgvDSKemTheo: {
        DataSource: [],
        cols: [
      ...
      dgvChiTietKM_new: {
        ...
        cols: [
          ...
          { fieldName: 'CUOC_SD', headerText: 'Giảm cước SD', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
          { fieldName: 'CUOC_TB', headerText: 'Giảm cước TB', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
          { fieldName: 'TYLE_KMLD', headerText: 'Tỷ lệ KMLĐ', format: this.is_nv_vnpti==1?'N2':'N', editType: 'numericedit', textAlign: 'right', allowFiltering: true, allowSorting: true, allowEditing: true },
          { fieldName: 'NGAY_BDDC', headerText: 'Ngày BĐĐC', allowFiltering: true, allowSorting: true, allowEditing: true, visible: true },
          { fieldName: 'NGAY_KTDC', headerText: 'Ngày KTĐC', allowFiltering: true, allowSorting: true, allowEditing: false, visible: true },
          { fieldName: 'NGAY_BD', headerText: 'Ngày BĐ', allowFiltering: true, allowSorting: true, allowEditing: false, visible: true },
          { fieldName: 'NGAY_KT', headerText: 'Ngày KT', allowFiltering: true, allowSorting: true, allowEditing: false, visible: true },
  ...
  async mounted() {
    ...
    if (this.khuyenmai_id > 0) {
      ...
      if (this.ts_loc_ct_tratruoc_dvgt == 1 && this.m_dsThueBao_DichVu) {
        ...
        var pdata = {
          ...
          loainha_id: this.dulieu.loainha_id?this.dulieu.loainha_id:0,
        }
      ...
      if (this.kieu_km == 0) {
        ...
        if (!this.dsKM_HDTB || this.dsKM_HDTB.length <= 0) {
          for (var item of this.dt) if (item['hienthi_md']) item['dangky'] = 1
        } else
          for (var item1 of this.dt)
            for (var item2 of this.dsKM_HDTB)
              if (item1['chitietkm_id'] == item2['chitietkm_id']) {
                // console.log(item2,item2["thang_bd"],item2["thang_bddc"],item2["thang_bdtg"]);
                item1['dangky'] = 1
        ...
        if (!this.dsDC_HDTB || this.dsDC_HDTB.length <= 0) {
          for (var item of this.dt) if (item['hienthi_md'] == 1) item['dangky'] = 1
        } else
          for (var item1 of this.dt)
            for (var item2 of this.dsDC_HDTB)
              if (item1['chitietkm_id'] == item2['chitietkm_id']) {
                item1['dangky'] = 1
                item1['thang_bdkm'] = item2['thang_bd']
                item1['thang_bddc'] = item2['thang_bddc']
      ...
      if (this.dt && this.dt.length > 0)
        for (var item of this.dt) {
          ...
          // item['ngay_bd'] = item['ngay_bd']?item['ngay_bd']:null;
          // item['ngay_bddc'] = item['ngay_bddc']?item['ngay_bddc']:null;
          // item['ngay_kt'] = item['ngay_kt']?item['ngay_kt']:null;
          // item['ngay_ktdc'] = item['ngay_ktdc']?item['ngay_ktdc']:null;
        }
    } else {
    ...
    if (this.dt && this.dt.length > 0) {
      for (var i = 0; i < this.dt.length; i++) {
        if (this.dt[i]['dangky']) {
          var chitietkm_id = this.dt[i]['chitietkm_id']
          var check_dvgt = await this.kiemtra(chitietkm_id);
          if (!check_dvgt) {
            this.$toast.error("Thuê bao chưa chọn dịch vụ GTGT được Khuyến mại. Hãy chọn lại dịch vụ cộng thêm!");
            return;
          }
        }
      }
  ...
  methods: {
      ...
      if (this.kieu_km == 0) {
        ...
        for (var item of this.dt) {
          ...
          if (item['dangky'] == 1) {
            ...
            if (ithang_huongdc > 0) dtien_td = this.round(dtiendatcoc_csd / ithang_huongdc)
            i_tien_td = dtien_td
        ...
        for (var item of this.dt) {
          var dr = {}
          if (item['dangky'] == 1) {
            var chitietkm_id = item['chitietkm_id']
            var check_dvgt = await this.kiemtra(chitietkm_id)
            if (!check_dvgt) {
              this.$toast.error("Thuê bao chưa chọn dịch vụ GTGT được Khuyến mại. Hãy chọn lại dịch vụ cộng thêm!");
              return;
            ...
            if (this.ht_kmtt) {
              ...
              var tien = this.round((tongtien * 100) / (100 + tyle_vat))
              var vat = this.round((tien * tyle_vat) / 100)
    ...
    kiemtra: async function (chitietkm_id) {
      try {
        var chk = true;
        // txtDoiTuongKM.Focus();
        let rs = await this.$root.context.get(`/web-hopdong/thaydoithongtinkhuyenmai/lay_ctkm_dvgt/${chitietkm_id}`);
        let dt1 = rs.data && rs.data.length > 0 ? this.UpperCasePropertyList(rs.data) : [];
        // dt1 = tchopdong.LAY_CTKM_DVGT(vchitietkm_id); 
        //Kiểm tra mục này có chi tiết khuyến mại không???
        if (dt1 && dt1.length > 0) //Nếu có chi tiết khuyến mại kèm dvgt. Kiểm tra khuyến mại đó đã có dịch vụ gia tăng chưa?
        {
          for (var i = 0; i < dt1.length; i++) {
            if (!this.kiemtra_dvgt(Number(dt1[i]["DICHVUGT_ID"])))
              chk = false;
            else
              return true;
          }
          // dgrChiTietKM.Rows[nIndex].Cells["DangKy"].Value = 0;      
          // if (!chk) {
          //   var nIndex = this.dt.findIndex(e => e['chitietkm_id'] == chitietkm_id)
          //   this.dt[nIndex]["dangky"] = 0
          // }             
        }
        return chk;
      } catch (e) {
    ...
    kiemtra_dvgt(vdichvugt_id) {
      var m_dsThueBao_DichVu = this.m_dsThueBao_DichVu ? this.UpperCasePropertyList(this.m_dsThueBao_DichVu) : [];
      if (m_dsThueBao_DichVu && m_dsThueBao_DichVu.length > 0) {
        var t = [];
        if (this.isGiaHan) {
          // t = ds.Select("DANGKY=1 or SUA=1 or (SUDUNG = 'Su dung' and HUY = 0)");
          t = m_dsThueBao_DichVu.filter(e => Number(e['DANGKY']) == 1 || Number(e['SUA']) == 1 || (e['SUDUNG'] == 'Su dung' && Number(e['HUY']) == 0))
        } else { 
          // t = ds.Select("DANGKY=1");
          t = m_dsThueBao_DichVu.filter(e => Number(e['DANGKY']) == 1)
        }

        if (t.length > 0) {
          for (var i = 0; i < t.length; i++) {
            if (vdichvugt_id == Number(t[i]["DICHVUGT_ID"])) return true;
          }
        }          
      }
      return false;
