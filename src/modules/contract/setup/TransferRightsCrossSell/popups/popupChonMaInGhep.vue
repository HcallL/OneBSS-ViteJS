        ...
        <div class="popup-body">
            <div class="box-form">
                ...
                <div class="row">
                    <div class="col-sm-4 col-12">
                        <div class="info-row">
                            ...
                            <div class="value">
                                <input type="text" class="form-control highlight" v-model="main_ghep">
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-sm-4 col-12">
                        <div class="info-row">
                            <PopupTimKiemNhanVienThuCuoc :input="diachiIG" @acceptChange="getNVTCProcess($event)" ref="dlgTimKiemNVTC_IG"></PopupTimKiemNhanVienThuCuoc>
...
export default {
    ...
    methods: {
        ...
        async HienThiDS() {
            try {
                var rs = await this.$root.context.post("/web-hopdong/thongtin_maghep/lay_ds_mig_hddb_kh", { "vhdkh_id": this.hdkh_id,"vkhachhang_id":this.khachhang_id,"vhdtt_id":this.hdtt_id });
        ...
        async dgvDanhSach_selectedRowChanged(item) {
            if(!item) return;
            console.log(item)
            this.listbox.dgvDanhSach.value = item.data;
            try {
                this.fhdmig_id = 0;
                this.fmig_id = 0;
                ...
                this.$bvModal.msgBoxConfirm('Bạn có muốn sinh mã in ghép không ?', {
                    ...
                    if(v) {
                        ...
                        if(rs.data) {
                            ...
                            TaoDuLieu_HDMIG(0);
                            TaoDuLieu_DiaChi();
                            var rs1 = await this.$root.context.post("/web-hopdong/lapdatmoi/fn_laytt_taodl_maig", { "ds_para": JSON.stringify({"THEMMOI":true,"HD_MIG":this.dsHDMIG,"DIACHI":this.dsDiaChi,"DIACHI_HDMIG":this.dsDiaChi_HDMIG,"HDTT_ID":this.hdtt_id}) });
                            if(rs1.data) {
        ...
        popupTimNVThuCuoc() {
            this.$refs.dlgTimKiemNVTC_IG.openDialog();
        },
    }
}
</script>
