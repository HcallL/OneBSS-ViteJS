...
export default {
  ...
  created: async function () {
    ...
    this.kt_tthd_kichhoat = gtts == 1
    this.soNgayThuHoi = await this.getTSMDByTS('NGAY_THUHOI')
    if (this.soNgayThuHoi != '') { this.ttPhieu.rdGiao = '1' } else this.ttPhieu.rdGiao = '0'
  },
  async mounted () {
  ...
  methods: {
    ...
    async closeKHModal (value) {
      ...
      if (value) {
        ...
        if (lstChosen.length > 0) {
          ...
          const errors = res.filter(x => x !== true)
          ...
          if (ok === res.length) {
            ...
            this.$toast.error(`Lỗi khi kích hoạt ${res.length} mục` + '\r\n' + errors.join('\r\n'))
          }
        }
      }
    },
    ...
    selectRow (value) {
      ...
      if (value) {
        ...
        this.ttPhieu.phieu_id = value.PHIEU_ID
        this.getDsNV()
    ...
    kichHoat: async function (body) {
      try {
        const res = await this.axios.put(`/web-thuno/api/thu-no/xu-ly-thue-bao/kich-hoat-dich-vu/hop-dong-thue-bao/${body.HDTB_ID}`)
        ...
        if (data && data.error == '200') {
          ...
          return '- ' + body.MA_TB + ' : ' + data.message_detail
        }
      } catch (error) {
        // console.log(error.data.message_detail)
        // this.$toast.error(error.data.message_detail)
        return '- ' + body.MA_TB + ' : ' + error.data.message_detail
        // return body.HDTB_ID
      }
    },
    ...
    capNhatNgay: async function (body) {
      ...
      try {
        ...
        if (data && data.error == '200') {
          ...
          await this.axios.post(`/app-cskh/TestPortV2/dokiem`, dokiemData)
          return true
        } else {
          // this.$toast.error('Kích hoạt thất bại!')
          // return false
