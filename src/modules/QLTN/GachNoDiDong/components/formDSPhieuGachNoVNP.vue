<template>
  <b-modal id="modal-dsphieugachnovnp" size="xl" hide-footer hide-header hide-header-close body-class="modal-body p-0"
    no-close-on-backdrop @hide="resetData" @show="openForm">
    <div class="modal-content popup-box">
      ...
      <div class="list-actions-top">
        <ul class="list">
          <li>
            <a href="#" @click.prevent="LayDS" title="Lấy danh sách phiếu thanh toán">
              <span class="icon one-file-attach1"></span>
              Lấy DS
            </a>
          </li>
          <li>
            <a href="#" v-on:click.prevent="XoaPhieu" :class="{ disabled: !thanhtoan_selected }" title="Xóa phiếu">
              <span class="icon one-file-trash"></span> Xóa phiếu
            </a>
          </li>
          <li>
            <a href="#" v-on:click.prevent="InBL" :class="{ disabled: dsPhieuThanhToan.length <= 0 }"
              title="In biên lai">
      ...
      <div class="popup-body">
        <div class="box-form">
          ...
          <div class="row">
            <div class="col-sm-3 col-12">
              <div class="info-row">
                ...
                <div class="value">
                  <input v-model="userGachNo" type="text" class="form-control highlight" @keyup.enter="enterUser" />
                </div>
              </div>
            </div>
            <div class="col-sm-2 col-12">
              <div class="info-row">
                ...
                <div class="value">
                  <input autofocus ref="maTT" v-model="maTT" type="text" class="form-control highlight"
            ...
            <div class="col-sm-3 col-12">
              <div class="info-row">
                ...
                <div class="value">
                  <input v-model="tienTra" type="text" class="form-control text-right" />
                </div>
              </div>
            </div>
          ...
          ]" :dataSource="dsPhieuThanhToan" :showFilter="true" @selectedRowChanged="thanhtoan_selectedRowChanged"
            ref="gridThanhToan" :allowPaging="true" :enablePagingServer="true" :totalRecords="totalItems"
            @pageChanged="thanhtoan_gridChanged" :key="keyReset" />
        </div>
      </div>
    </div>
  </b-modal>
  ...
  methods: {
    ...
    async fetchXoaPhieu() {
      ...
      try {
        ...
        const response = await API2.xoaPhieuDHSX(this.axios, data);
