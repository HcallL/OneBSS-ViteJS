...
export default {
  ...
  methods: {
    refresh () {
      ...
      this.model.cboNT_DV_BAN = {list: [], value: []}
      this.model.txtSNCP_BAN = ''
      this.model.txtMTU_Size_BAN = ''
      this.model.txtAuto_Nego_BAN = ''
      this.model.txtCapDP_BAN = ''
      this.model.txtCapChinh_BAN = ''
      ...
      this.model.cboNT_DV_TC = {list: [], value: []}
      this.model.txtSNCP_TC = ''
      this.model.txtMTU_Size_TC = ''
      this.model.txtAuto_Nego_TC = ''
      this.model.txtCapDP_TC = ''
      this.model.txtCapChinh_TC = ''
    ...
    async openDialog (tinh, ploaitb_ld, ploaitb, kieuld_id, pthongso_Ban, pthongso_TC, vloaihd, vkenhtrang) {
      try {
        ...
        console.log('🚀 ~ file: frmThongSoTSL.vue:230 ~ openDialog ~ this.thongso_Ban:', this.thongso_Ban)
        this.thongso_TC = pthongso_TC
        console.log('🚀 ~ file: frmThongSoTSL.vue:232 ~ openDialog ~ this.thongso_TC:', this.thongso_TC)
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_Ban.doituong_id == undefined || this.thongso_Ban.doituong_id == 0) { this.model.cboDoituongBan.value = temps[0].doituong_id } else { this.model.cboDoituongBan.value = this.thongso_Ban.doituong_id }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_TC.doituong_id == undefined || this.thongso_TC.doituong_id == 0) { this.model.cboDoituongTC.value = temps[0].doituong_id } else { this.model.cboDoituongTC.value = this.thongso_TC.doituong_id }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_TC.kieuld_id == undefined || this.thongso_TC.kieuld_id == 0) { this.model.cboKieuLD.value = this.model.cboKieuLD.list[0].id } else { this.model.cboKieuLD.value = this.thongso_TC.kieuld_id }
        } else { this.$toast.error('Không tìm thấy kiểu lắp đặt ở bên thi công ứng với loại hợp đồng và loại thuê bao này.') }

        // Loai kenh
        // temps = await this.Laythongtin_table('css.loai_kenh', 'loaikenh_id, loai_kenh', '1=1 order by loaikenh_id', '')
        temps = await this.Laythongtin_table_loai_kenh()
        if (temps && temps.length > 0) {
          this.model.cboLoaikenhBan.list = temps.map((x) => ({id: x.loaikenh_id, text: x.loai_kenh}))
          if (this.thongso_Ban.loaikenh_id == undefined || this.thongso_Ban.loaikenh_id == 0) { this.model.cboLoaikenhBan.value = temps[0].loaikenh_id } else { this.model.cboLoaikenhBan.value = this.thongso_Ban.loaikenh_id }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_TC.loaikenh_id == undefined || this.thongso_TC.loaikenh_id == 0) { this.model.cboLoaikenhTC.value = temps[0].loaikenh_id } else { this.model.cboLoaikenhTC.value = this.thongso_TC.loaikenh_id }
        }
        ...
        let det = []
        if (this.tinh_chuquan_id == '-1') { det = ds_cq_ban.filter(item => item.tinh_id == this.$root.token.getPhanVungID()) } else { det = ds_cq_ban.filter(item => item.tinh_id == this.tinh_chuquan_id) }

        if (det.length > 0) {
          let row = det[0]
          if (row.chuquan_id != undefined && row.chuquan_id != '') { this.model.cboChuquanBan.value = Number(row.chuquan_id) }
        }

        det = ds_cq_ban.filter(item => item.tinh_id == this.tinhtc.tinhthicong_id)
        if (det.length > 0) {
          let row = det[0]
          if (row.chuquan_id != undefined && row.chuquan_id != '') { this.model.cboChuQuanLD_Ban.value = Number(row.chuquan_id) }
        }

        if (this.tinh_chuquan_id == '-1') { det = ds_cq_tc.filter(item => item.tinh_id == this.$root.token.getPhanVungID()) } else { det = ds_cq_tc.filter(item => item.tinh_id == this.tinh_chuquan_id) }

        if (det.length > 0) {
          let row = det[0]
          if (row.chuquan_id != undefined && row.chuquan_id != '') { this.model.cboChuquanTC.value = Number(row.chuquan_id) }
        }

        det = ds_cq_tc.filter(item => item.tinh_id == this.tinhtc.tinhthicong_id)
        if (det.length > 0) {
          let row = det[0]
          if (row.chuquan_id != undefined && row.chuquan_id != '') { this.model.cboChuQuanLD_TC.value = Number(row.chuquan_id) }
        }

        // Toc do kenh
        // temps = await this.Laythongtin_table('css.tocdo_kenh', "tocdo_id, tocdo||' '||donvi toc_do", '1=1 order by tocdo', '')
        temps = await this.Laythongtin_table_tocdo_kenh()
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_Ban.tocdo_id == undefined || this.thongso_Ban.tocdo_id == 0) { this.model.cboTocdoBan.value = temps[0].tocdo_id } else { this.model.cboTocdoBan.value = this.thongso_Ban.tocdo_id }

          if (this.thongso_Ban.tocdo_pop != undefined && this.thongso_Ban.tocdo_pop > 0) { this.model.cboTocDoPOP_BAN.value = this.thongso_Ban.tocdo_pop }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_TC.tocdo_id == undefined || this.thongso_TC.tocdo_id == 0) { this.model.cboTocdoTC.value = temps[0].tocdo_id } else { this.model.cboTocdoTC.value = this.thongso_TC.tocdo_id }

          if (this.thongso_TC.tocdo_pop != undefined && this.thongso_TC.tocdo_pop > 0) { this.model.cboTocDoPOP_TC.value = this.thongso_TC.tocdo_pop }
        }

        // Trang bi
        // temps = await this.Laythongtin_table('css.trangbi', 'trangbi_id, tentrangbi', '1=1 order by trangbi_id', '')
        temps = await this.Laythongtin_table_trangbi()
        if (temps && temps.length > 0) {
          this.model.cboTrangbiBan.list = temps.map((x) => ({id: x.trangbi_id, text: x.tentrangbi}))
          if (this.thongso_Ban.trangbi_id == undefined || this.thongso_Ban.trangbi_id == 0) { this.model.cboTrangbiBan.value = temps[0].trangbi_id } else { this.model.cboTrangbiBan.value = this.thongso_Ban.trangbi_id }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_TC.trangbi_id == undefined || this.thongso_TC.trangbi_id == 0) { this.model.cboTrangbiTC.value = temps[0].trangbi_id } else { this.model.cboTrangbiTC.value = this.thongso_TC.trangbi_id }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_Ban.muccuoctb_id == undefined || this.thongso_Ban.muccuoctb_id == 0) { this.model.cboMuccuoctbBan.value = temps[0].muccuoctb_id } else { this.model.cboMuccuoctbBan.value = this.thongso_Ban.muccuoctb_id }
        }
        ...
        if (temps && temps.length > 0) {
          ...
          if (this.thongso_TC.muccuoctb_id == undefined || this.thongso_TC.muccuoctb_id == 0) { this.model.cboMuccuoctbTC.value = temps[0].muccuoctb_id } else { this.model.cboMuccuoctbTC.value = this.thongso_TC.muccuoctb_id }
        }
        ...
        // Mo hinh kn
        // temps = await this.Laythongtin_table('css.mohinh_kn', 'mhkn_id, mohinh_kn', 'phanvung_id = ' + this.$root.token.getPhanVungID(), '')
        temps = await this.lay_thongtin_ds_mohinh_kn()
        if (temps && temps.length > 0) {
          this.model.cboMHKN_BAN.list = temps.map((x) => ({id: x.mhkn_id, text: x.mohinh_kn}))
          if (this.thongso_Ban.mhkn_id != undefined && this.thongso_Ban.mhkn_id > 0) { this.model.cboMHKN_BAN.value = this.thongso_Ban.mhkn_id }
        }
        temps = await this.HT_MoHinh_KN_Combobox(this.tinhtc.tinhthicong_id)
        if (temps && temps.length > 0) {
          this.model.cboMHKN_TC.list = temps.map((x) => ({id: x.mhkn_id, text: x.mohinh_kn}))
          if (this.thongso_TC.mhkn_id != undefined && this.thongso_TC.mhkn_id > 0) { this.model.cboMHKN_TC.value = this.thongso_TC.mhkn_id }
        }

        // Giao dien kn
        // temps = await this.Laythongtin_table('css.giaodien_kn', 'gdkn_id, giaodien_kn', 'phanvung_id = ' + this.$root.token.getPhanVungID(), '')
        temps = await this.lay_thongtin_ds_giaodien_kn()
        if (temps && temps.length > 0) {
          this.model.cboGiaoDienKH_BAN.list = temps.map((x) => ({id: x.gdkn_id, text: x.giaodien_kn}))
          if (this.thongso_Ban.gdkn_id != undefined && this.thongso_Ban.gdkn_id > 0) { this.model.cboGiaoDienKH_BAN.value = this.thongso_Ban.gdkn_id }
        }
        temps = await this.HT_GiaoDien_KN_Combobox(this.tinhtc.tinhthicong_id)
        if (temps && temps.length > 0) {
          this.model.cboGiaoDienKH_TC.list = temps.map((x) => ({id: x.gdkn_id, text: x.giaodien_kn}))
          if (this.thongso_TC.gdkn_id != undefined && this.thongso_TC.gdkn_id > 0) { this.model.cboGiaoDienKH_TC.value = this.thongso_TC.gdkn_id }
        }

        // Ngiem thu DV
        // this.model.cboNT_DV_BAN.list = this.list2treeNew(await this.Laythongtin_table('css.kieu_nghiemthu a', '0 chon, a.kieunt_id, a.kieu_nghiemthu', '1=1', ''))
        this.model.cboNT_DV_BAN.list = this.list2treeNew(await this.lay_thongtin_ds_kieu_nghiemthu())
        if (this.thongso_Ban.kieu_nghiemthu != undefined && this.thongso_Ban.kieu_nghiemthu != null &&
                        this.thongso_Ban.kieu_nghiemthu.length > 0) {
          for (let i = 0; i < this.model.cboNT_DV_BAN.list.length; i++) {
            let ff = this.thongso_Ban.kieu_nghiemthu.filter(item => item.kieunt_id == this.model.cboNT_DV_BAN.list[i].kieunt_id)
            if (ff.length > 0) {
              this.model.cboNT_DV_BAN.value.push(this.model.cboNT_DV_BAN.list[i].kieunt_id)
            }
          }
        }

        // this.model.cboNT_DV_TC.list = this.list2treeNew(await this.Laythongtin_table('css.kieu_nghiemthu a', '0 chon, a.kieunt_id, a.kieu_nghiemthu', '1=1', ''))
        this.model.cboNT_DV_TC.list = this.list2treeNew(await this.lay_thongtin_ds_kieu_nghiemthu())
        if (this.thongso_TC.kieu_nghiemthu != undefined && this.thongso_TC.kieu_nghiemthu != null &&
                        this.thongso_TC.kieu_nghiemthu.length > 0) {
          for (let i = 0; i < this.model.cboNT_DV_TC.list.length; i++) {
            let ff = this.thongso_TC.kieu_nghiemthu.filter(item => item.kieunt_id == this.model.cboNT_DV_TC.list[i].kieunt_id)
            if (ff.length > 0) {
              this.model.cboNT_DV_TC.value.push(this.model.cboNT_DV_TC.list[i].kieunt_id)
            }
          }
        }
        ...
        if (kq == 'OK') { this.ht_tskt_vnpti = true }
        ...
        this.kt_load = false
        if ((vloaihd == Enum.LoaiHopDong.CHUYEN_QUYEN || vloaihd == Enum.LoaiHopDong.DI_CHUYEN) && !this.kenhtrang) {
    ...
    async Laythongtin_table_loai_kenh () {
      let res = []
      try {
        let params = {
          p_type: 1, p_param: ''
        }
        let response = await api.lay_thongtin_sp_tt_loai_kenh(this.axios, params)
        console.log(response)
        if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
          res = response.data.data
        }
    ...
    async Laythongtin_table_tocdo_kenh () {
      let res = []
      try {
        let params = {
          p_type: 2, p_param: ''
        }
        let response = await api.lay_thongtin_sp_tt_tocdo_kenh(this.axios, params)
        console.log(response)
        if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
          res = response.data.data
        }
    ...
    async Laythongtin_table_trangbi () {
      let res = []
      try {
        let params = {
          p_type: 1, p_param: ''
        }
        let response = await api.lay_thongtin_sp_tt_trangbi(this.axios, params)
        console.log(response)
        if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
          res = response.data.data
        }
    ...
    async lay_thongtin_ds_mohinh_kn () {
      ...
      try {
        ...
        let response = await api.lay_thongtin_ds_mohinh_kn(this.axios, params)
        console.log(response)
        if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
          res = response.data.data
        }
    ...
    async lay_thongtin_ds_giaodien_kn () {
      ...
      try {
        ...
        let response = await api.lay_thongtin_ds_giaodien_kn(this.axios, params)
        console.log(response)
        if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
          res = response.data.data
        }
    ...
    async lay_thongtin_ds_kieu_nghiemthu () {
      ...
      try {
        ...
        let response = await api.lay_thongtin_ds_kieu_nghiemthu(this.axios, params)
        console.log(response)
        if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
          res = response.data.data
        }
    ...
    async Lay_DS_MucCuoc_TB (tinhthicong_id, khuld_id, apld_id, phold_id, phuongld_id, loaitb_id, doituong_id, ngay, trangbi_id,
      ...
      if (loaitb_id == Enum.LoaiHinhTB.INTERNET_FTTH ||
                ...
                loaitb_id == Enum.LoaiHinhTB.INTERNET_ADSL ||
                loaitb_id == Enum.LoaiHinhTB.INTERNET_ADSL_TINH_KHAC ||
                loaitb_id == Enum.LoaiHinhTB.INTERNET_FIBER_TINH_KHAC) {
        vtocdo_id = tocdo_id
        vmuccuoc_id = muccuoc_id
      } else if (loaitb_id == Enum.LoaiHinhTB.INTERNET_MYTV) {
        vtocdo_id = tocdo_id
        vmuccuoc_id = 0
      } else if (loaitb_id == Enum.LoaiHinhTB.FSECURE) { // Lamnt: bổ sung hiển thị mức cước theo tốc độ thuê bao FSecure (09/03/2017)
        vtocdo_id = tocdo_id
        vmuccuoc_id = 0
      }
    ...
    async cboTocdoBan_SelectedIndexChanged () {
      if (!this.kt_load) {
        ...
        if (!kt_tocdo_tc) { this.$toast.error('Tốc độ không có bên tỉnh thi công !') }
    ...
    async tsbtnCapNhat_Click () {
      try {
        ...
        if (Number(this.model.cboThietbiCuoiBan.value) == 0 || Number(this.model.cboThietbiDauBan.value) == 0 ||
                    Number(this.model.cboThietbiDauTC.value) == 0 || Number(this.model.cboThietbiCuoiTC.value) == 0) {
        ...
        if (Number(this.model.cboLoaikenhBan.value) == 9) { // kênh quốc tế
          try {
            ...
            // console.log(dt_kieu_nt_ban, dt_kieu_nt_tc)
            if ((dt_kieu_nt_tc === null || dt_kieu_nt_ban === null || dt_kieu_nt_tc.length === 0 ||dt_kieu_nt_ban.length ===0) 
              && this.vloaihd_id == Enum.LoaiHopDong.DAT_MOI &&
        ...
        if (this.model.cboLoaikenhBan.list.length > 0) { this.thongso_Ban.loaikenh_id = Number(this.model.cboLoaikenhBan.value) }
        if (this.model.cboMuccuoctbBan.list.length > 0) { this.thongso_Ban.muccuoctb_id = Number(this.model.cboMuccuoctbBan.value) }
        ...
        if (this.model.cboLoaikenhTC.list.length > 0) { this.thongso_TC.loaikenh_id = Number(this.model.cboLoaikenhTC.value) }
        if (this.model.cboMuccuoctbTC.list.length > 0) { this.thongso_TC.muccuoctb_id = Number(this.model.cboMuccuoctbTC.value) }
        ...
        if (this.model.cboThietbiDauTC.value != null) { this.thongso_TC.thietbidc_id = Number(this.model.cboThietbiDauTC.value) }
        if (this.model.cboThietbiCuoiTC.value != null) { this.thongso_TC.thietbidc_id_cuoi = Number(this.model.cboThietbiCuoiTC.value) }

        if (this.model.cboMHKN_TC.value != '' && this.model.cboMHKN_TC.value != '0') { this.thongso_Ban.mhkn_id = Number(this.model.cboMHKN_TC.value) } else { this.thongso_Ban.mhkn_id = 0 }
        if (this.model.cboMHKN_TC.value != '' && this.model.cboMHKN_TC.value != '0') { this.thongso_TC.mhkn_id = Number(this.model.cboMHKN_BAN.value) } else { this.thongso_TC.mhkn_id = 0 }

        if (this.model.cboTocDoPOP_BAN.value != '' && this.model.cboTocDoPOP_BAN.value != '0') { this.thongso_Ban.tocdo_pop = Number(this.model.cboTocDoPOP_BAN.value) } else { this.thongso_Ban.tocdo_pop = 0 }
        if (this.model.cboTocDoPOP_TC.value != '' && this.model.cboTocDoPOP_TC.value != '0') { this.thongso_TC.tocdo_pop = Number(this.model.cboTocDoPOP_TC.value) } else { this.thongso_TC.tocdo_pop = 0 }

        if (this.model.cboGiaoDienKH_BAN.value != '' && this.model.cboGiaoDienKH_BAN.value != '0') { this.thongso_Ban.gdkn_id = Number(this.model.cboGiaoDienKH_BAN.value) } else { this.thongso_Ban.gdkn_id = 0 }
        if (this.model.cboGiaoDienKH_TC.value != '' && this.model.cboGiaoDienKH_TC.value != '0') { this.thongso_TC.gdkn_id = Number(this.model.cboGiaoDienKH_TC.value) } else { this.thongso_Ban.gdkn_id = 0 }

        this.thongso_Ban.kieu_nghiemthu = dt_kieu_nt_ban
        this.thongso_TC.kieu_nghiemthu = dt_kieu_nt_tc

        this.thongso_Ban.cap_chinh = this.model.txtCapChinh_BAN
        this.thongso_TC.cap_chinh = this.model.txtCapChinh_TC

        this.thongso_Ban.cap_duphong = this.model.txtCapDP_BAN
        this.thongso_TC.cap_duphong = this.model.txtCapDP_TC

        this.thongso_Ban.auto_nego = this.model.txtAuto_Nego_BAN
        this.thongso_TC.auto_nego = this.model.txtAuto_Nego_TC

        this.thongso_Ban.mtu_size = this.model.txtMTU_Size_BAN
        this.thongso_TC.mtu_size = this.model.txtMTU_Size_TC

        this.thongso_Ban.sncp = this.model.txtSNCP_BAN
        this.thongso_TC.sncp = this.model.txtSNCP_TC
