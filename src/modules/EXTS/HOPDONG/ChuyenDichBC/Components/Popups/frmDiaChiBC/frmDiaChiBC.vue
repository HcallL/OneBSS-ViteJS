  ...
  methods: {
    ...
    async cboQuan_SelectedValueChanged () {
      try {
        ...
        console.log('🚀 ~ file: frmDiaChiBC.vue ~ line 337 ~ cboQuan_SelectedValueChanged ~ error', error)
      }
    },
    async HienThiTTDiaChi () {
      try {
        ...
        if (this.them_text_vietnam === 1) { this.model.txtDiaChi = _sSoNha + sKhu + sAp + sDuong + phuong + quan + ', Việt Nam' } else { this.model.txtDiaChi = _sSoNha + sKhu + sAp + sDuong + phuong + quan }
      } catch (error) {
        console.log('🚀 ~ file: frmDiaChiBC.vue ~ line 386 ~ HienThiTTDiaChi ~ error', error)
      }
    },
