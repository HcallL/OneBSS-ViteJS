...
export default {
    ...
    methods: {
        ...
        async HienThiTT_Datcoc() {
            ...
            let postData = {
                p_data: JSON.stringify({"thuebao_id" : this.thuebao_id})
            }
            let ds_datcoc = await this.LAY_DS_DATCOC_THEO_MATB(postData)

            console.log(ds_datcoc)
            this.dgvDatCoc.DataSource = [...ds_datcoc]
        },
        ...
        async TaoDuLieu(themmoi) {
            ...
            this.nhieu_tb = this.dgvDatCoc.DataSource.length > 1 ? true : false
        ...
        async TaoDuLieu_HDTB_DATCOC_v2(themmoi) {
            ...
            for (let i = 0; i < this.dgvDatCoc.DataSource.length; i++) {
                ...
                let r = this.dgvDatCoc.DataSource[i]
                let thuebao_dc_id = r.THUEBAO_DC_ID

                if (themmoi == true) {
                    thuebao_dc_id = await this.GetKey("THUEBAO_DC_ID")
                }

                let node = doc.createElement("item")

                // node.setAttribute("thuebao_dc_id", this.thuebao_dc_id)
                node.setAttribute("thuebao_dc_id", thuebao_dc_id)
        ...
        async TaoDuLieu_CT_TIENHD_v2(themmoi) {
            ...
            for (let i = 0; i < this.dgvDatCoc.DataSource.length; i++) {
                ...
                let r = this.dgvDatCoc.DataSource[i]
                let tb_huy = this.dgvDanhSach.DataSource.find(x => x.THUEBAO_ID === r.THUEBAO_ID)
                let v_ct_tienhd_id = null

                if (themmoi == true) {
                    // this.CT_TIENHD_ID = await this.GetKey("CT_TIENHD");
                    v_ct_tienhd_id = await this.GetKey("CT_TIENHD");
                } else {
                    v_ct_tienhd_id = tb_huy.CT_TIENHD_ID
                }
                let tienthoai = 0;
                ...
                node.setAttribute("hdtb_id", parseInt(this.dsHDTB.HD_THUEBAO[i]["hdtb_id"]))
                // node.setAttribute("ct_tienhd_id", this.CT_TIENHD_ID)
                node.setAttribute("ct_tienhd_id", v_ct_tienhd_id)
                node.setAttribute("khoanmuctt_id", this.KHOANMUC_TT.KMTT_DATCOC)
                node.setAttribute("id", this.KHOANMUC_TT.KMTT_DATCOC)
                node.setAttribute("loai_id", 1)
                node.setAttribute("tien", -tien)
        ...
        async lay_ngay_cn() {
            try {
                if (this.ngay_cn !== null)  return this.ngay_cn
                ...
                this.ngay_cn = !response.data ? null : response.data.ngay_cn
                return this.ngay_cn
            } catch (err) {
                return null
            }
