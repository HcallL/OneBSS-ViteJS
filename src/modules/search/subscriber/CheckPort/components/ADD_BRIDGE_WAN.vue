...
export default {
  ...
  validations: {
     txtPriority: {
      validPriority: function (Priority) {
        return Priority && Priority >= 0 && Priority <= 7;
      }
     //^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$@!%&*?])[A-Za-z\d#$@!%&*?]{8,30}$/
     //Length: maxLength(200)
    },
    txtVlanId: {
      validVlanId: function (VlanId) {
        return VlanId && VlanId >= 0 && VlanId <= 4094;
      }
     //^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$@!%&*?])[A-Za-z\d#$@!%&*?]{8,30}$/
     //Length: maxLength(200)
    }
  ...
  methods: {
    ...
    Clear() {
      ...
      // this.chkMacAddress = null;
      // this.txtUserONT = null;
      // this.txtPassONT = null;
      //this.txtResult = null;
    },
    async tracuuport() {
        this.loading(true)
...
.addBridgeWan .ml-2{padding-left: 5px;}
.addBridgeWan .mb-2{padding-bottom: 3px;}

</style>
<template>
  <div class="addBridgeWan">
      <b-row>
          <b-col sm="3" cols="12">
            <div class="key">802.1P Priority [0-7]</div>
            </b-col>
          <b-col sm="3" cols="12">
            <div class="value">
                  <input type="number" v-model= "txtPriority" ref="textFocus" class="form-control">
            </div>
            <div class="text-sm mt-2 text-danger ml-2" v-if="$v.txtPriority.$error">
                <div class="error" v-if="!$v.txtPriority.validPriority">Priority [0-7]</div>
            </div>
          </b-col>
      </b-row>
      <b-row>
          <b-col sm="3" cols="12">
            <div class="key">802.1Q VLAN ID [0-4094]</div>
          </b-col>
          <b-col sm="3" cols="12">
              <div class="value">
                <input type="number" v-model= "txtVlanId" labelWidth="0" ref="textFocusvlanid" class="form-control">
              </div>
              <div class="text-sm mt-2 text-danger ml-2" v-if="$v.txtVlanId.$error">
                        <div class="error" v-if="!$v.txtVlanId.validVlanId">VLAN ID [0-4094]</div>
            </div>
          </b-col>
      </b-row>
      <b-row>
