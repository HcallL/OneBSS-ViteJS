...
export default {
  ...
  validations: {
    txtUserONT: {
     ...
     validTenONT: function (TenONT) {
       var Regex = new RegExp("^[a-z](?:_?[a-z0-9._ -]+)*$");
        var res = Regex.test(TenONT);
        return res;
      }
    },
     txtPasswordONT: {
...
.changeONT .ml-2{padding-left: 5px;}
.changeONT .mb-2{padding-bottom: 3px;}
...
<template>
  <div class="changeONT">
      <b-row>
          <b-col sm="2" cols="12">
            <div class="key">User ONT</div>
            </b-col>
          <b-col sm="3" cols="12">
            <div class="value pl-2 mb-2">
                  <input type="text" v-model= "txtUserONT" labelWidth="0" ref="textFocus" class="form-control">
            </div>
            <div class="text-sm mt-2 text-danger ml-2 mb-2" v-if="$v.txtUserONT.$error">
                       <div class="error" v-if="!$v.txtUserONT.required">Chưa nhập thông tin user ONT</div>
                        <div class="error" v-if="!$v.txtUserONT.maxLength">User ONT không được quá 30 ký tự</div>
                        <div class="error" v-if="!$v.txtUserONT.validTenONT">User ONT chứa ký tự không hợp lệ</div>
              </div>
          </b-col>
      </b-row>
      <b-row>
          <b-col sm="2" cols="12">
            <div class="key">Password ONT</div>
          </b-col>
          <b-col sm="3" cols="12">
              <div class="value pl-2">
                <input type="text" v-model= "txtPasswordONT" labelWidth="0" ref="textFocuspwd" class="form-control">
              </div>
              <div class="text-sm mt-2 text-danger ml-2" v-if="$v.txtPasswordONT.$error">
                        <div class="error" v-if="!$v.txtPasswordONT.required">Chưa nhập mật khẩu ONT</div>
                        <!-- <div class="error" v-if="!$v.txtPwd.minLength">Mật khẩu wifi phải có tối thiểu 8 ký tự và tối đa 30 ký tự</div> -->
                        <div class="error" v-if="!$v.txtPasswordONT.validPassword">Mật khẩu ONT phải có tối thiểu 8 ký tự và tối đa 30 ký tự</div>
                </div>
          </b-col>
      </b-row>
      <b-row>
          ...
          <b-col sm="2" cols="12">
            <b-button variant="primary" v-on:click="tracuuport" :disabled="UiState.btn_disable">Thực hiện</b-button>
              <!-- <vue-nav>
                <ul class="list">
                  <li @click="tracuuport">
