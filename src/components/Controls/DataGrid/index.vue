 ...
 * @author: IT3.Web2
 */
import { Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate, ContextMenu, DetailRow, Toolbar } from '@syncfusion/ej2-vue-grids'
...
export default {
  ...
  provide: {
    grid: [Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate, ContextMenu, DetailRow, Toolbar]
  },
  props: {
    columns: {
      type: Array,
  ...
  computed: {
    ...
    filteredDataSource: function() {
      ...
      if (result != null) {
        if (!(this.filterConditions == null || this.filterConditions.length == 0)) {
          this.filterConditions.forEach((filterCondition) => {
            result = result.filter((x) =>
              x[filterCondition.fieldName] == null || x[filterCondition.fieldName] == undefined
                ? false
                : filterCondition.value == null || filterCondition.value == undefined
                ? true
    ...
    selectionOptions: function() {
      ...
      if (this.showColumnCheckbox) {
        result.type = this.selectionSettings && this.selectionSettings.type == 'Single' ? 'Single' : 'Multiple'
      ...
      if (this.filteredDataSource != null) {
        if (this.allowPaging) {
          if (this.enablePagingServer) {
            ...
            result = this.filteredDataSource.slice(this.pageIndexData * this.pageSizeData, this.pageIndexData * this.pageSizeData + this.pageSizeData)
          }
      ...
      if (this.columns != null) {
        for (let i = 0; i < this.columns.length; i++) {
          let column = {
            ...
            columns: this.columns[i].columns == undefined ? null : this.columns[i].columns,
    ...
    useAggregates: function() {
      return !(this.aggregates == undefined || this.aggregates == null || this.aggregates.length == 0)
    },
  ...
  methods: {
    ...
    getFilterTemplate: function(column) {
      if (!(column.commands == null || column.commands == undefined || column.commands.length == 0) || column.allowFiltering === false) {
        return '<span></span>'
    ...
    getColumnCustomAttributes: function(column) {
      ...
      if (!(css == null || css == undefined || css == '')) {
        if (customAttributes == null || customAttributes == undefined) customAttributes = {}
        if (!(customAttributes.class == null || customAttributes.class == undefined)) customAttributes.class = ' ' + css
      ...
      if (!(this.commands == null || this.commands.length == 0)) {
        for (let i = 0; i < this.commands.length; i++) {
          if (this.commands[i].visible != false) {
            ...
            if (!(this.commands[i].cssClass == undefined || this.commands[i].cssClass == '')) cssClass += ' ' + this.commands[i].cssClass
            let text = ' '
            if (!(this.commands[i].text == undefined || this.commands[i].text == '')) text = this.commands[i].text
            let cmd = {
    ...
    getDataValueType: function(value) {
      ...
      if (!(value == null || value == undefined)) {
        ...
        else if (value.match(regNumber) || (typeof value === 'number' && isFinite(value))) type = 'Number'
        else type = 'String'
      }
      return type
    },
    getColumnType: function(column) {
      ...
      if (this.dataItems) {
        for (let i = 0; i < this.dataItems.length; i++) {
          ...
          if (row && !(row[column.fieldName] == undefined || row[column.fieldName] == null || row[column.fieldName] === '')) {
    ...
    getColumnTextAlign: function(column) {
      if (column.textAlign == undefined || column.textAlign == null) {
        ...
        if (dataValueType == 'Date' || dataValueType == 'Number') return 'right'
        return ''
    ...
    getSelectedValues: function(selectedRecords) {
      ...
      if (selectedRecords != null) {
        for (let i = 0; i < selectedRecords.length; i++) {
          ...
          if (this.dataKeyField.trim() != '') result.push(item[this.dataKeyField.trim()])
    ...
    getCellCreatedArgs: function(fieldName, cssClass, cellAllowHtml, text, cellElement, column, dataItem) {
      return {
    ...
    grid_RowSelected: function(args) {
      ...
      let args_tmp = args
      if (args && args.isHeaderCheckboxClicked) {
        records = this.filteredDataSource
        args_tmp.data = this.filteredDataSource
        args_tmp.rowIndexes = this.filteredDataSource.map((e, index) => index)
      }
      ...
      this.$emit('rowSelected', args_tmp)
    },
    ...
    grid_DataBound: function(args) {
      ...
      if (!this.flagBeginFilter) {
        if (this.defaultColumnCheckboxChecked == true) {
          this.flagSelectedRowIndexes = []
          let rows = this.filteredDataSource
          for (let i = 0; i < rows.length; i++) {
            this.flagSelectedRowIndexes.push(i)
          }
        }
        ...
        if (gridColumns.length > 0) {
          ...
          for (let i = 0; i < rows.length; i++) {
            ...
            for (let j = 0; j < gridColumns.length; j++) {
              ...
              if (!(gridColumn.field == undefined || gridColumn.field == null || gridColumn.field == '')) {
                let cell = this.grid.getCellFromIndex(row.rowIndex, gridColumn.index)
                let cssClass = ''
                let text = ''
                let cellItem = $(cell)
                if (!(cellItem == undefined || cellItem == null || cellItem.length == 0)) {
                  cssClass = cellItem[0].className
                  text = cellItem[0].outerText
                }
                let arg = this.getCellCreatedArgs(gridColumn.field, cssClass, gridColumn.disableHtmlEncode == false, text, cell, gridColumn, row.rowData)
    ...
    getModuleName: function() {
      if (!(this.$route.matched == null || this.$route.matched.length == 0)) return this.$route.matched[0].name
      else return 'ecms'
    },
    ...
    setFilterCondition: function(args) {
      ...
      let condition = this.filterConditions.find((x) => x.fieldName == args.fieldName)
    ...
    clearFilterCondition: function(fieldName) {
      this.filterConditions = this.filterConditions.filter((x) => x.fieldName != fieldName)
      this.currentFilter = {}
      this.$emit('filterChanged', {
        filters: this.filterConditions,
        current: this.currentFilter
    ...
    getFilterConditionValue: function(fieldName) {
      let result = null
      let condition = this.filterConditions.find((x) => x.fieldName == fieldName)
      if (!(condition == null || condition == undefined)) {
    ...
    toolbarClick: function(args) {
      if (args.item.id && args.item.id.includes('excelexport')) {
