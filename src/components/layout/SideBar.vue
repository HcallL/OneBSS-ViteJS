<template>
  <div class="sidebar">
    <div class="sidebar-content">
      ...
      <div class="search" v-if="lstChuyenTinh && lstChuyenTinh.length>1">
        <select2 class="bor0 nocorner bg-none" v-model="currentPhanVung" :options="lstChuyenTinh" @change="ChuyenPhanVung"></select2>
        <!-- <select
          v-model="txtSearch"
          placeholder="Chuyển vùng"
          class="form-control"
        /> -->
      </div>
...
export default {
  ...
  created: function () {
    ...
    EventBus.$on('emitChuyenTinh', this.emitChuyenTinh);
  },
  computed: {
    menuHtml: function () {
  ...
  methods: {
    emitChuyenTinh: function({id, reload}) {
      if (id != this.currentPhanVung) {
        this.ChuyenPhanVung(id, reload)        
      }
    },
    loadChuyenTinh: async function() {
      let ck = await this.$root.context.postWithOutToken('/quantri/user/tinhthanh_nguoidung', {username: this.$root.token.getUserName()});
      if(ck && ck.data && ck.data.length>1) {
        this.lstChuyenTinh = ck.data.map(x=>({ "id":x.phanvung_id, "text":(x.mavung?(x.mavung+"-"):"")+x.tentinh}));
        this.currentPhanVung = this.$root.token.getPhanVungID();
      } else this.lstChuyenTinh = [];
    },
    ChuyenPhanVung: async function(val, reload = true) {
      console.log(val);
      console.log(reload);
  ...
  destroyed() {
    ...
    EventBus.$off('emitChuyenTinh', this.emitChuyenTinh);
  },
};
</script>
